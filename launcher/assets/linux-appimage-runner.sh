...
if [ "$#" -lt 1 ]; then
  echo "MoreCodex AppImage runner requires an AppImage path" >&2
  exit 64
fi
APPIMAGE_PATH="$1"
shift
case "$APPIMAGE_PATH" in
  /*) ;;
  *) echo "MoreCodex AppImage path must be absolute" >&2; exit 64 ;;
esac
if [ ! -f "$APPIMAGE_PATH" ] || [ ! -x "$APPIMAGE_PATH" ]; then
  echo "MoreCodex AppImage is unavailable: $APPIMAGE_PATH" >&2
  exit 66
fi
...
if [ ! -x "$APP_DIR/AppRun" ]; then
  echo "MoreCodex AppImage fallback extraction produced no executable AppRun" >&2
  exit 70
fi
