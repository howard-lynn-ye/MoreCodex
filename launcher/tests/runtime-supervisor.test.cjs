...
test("Linux autostart launches the durable AppImage invisibly", () => {
  const entry = linuxDesktopEntry(
    ...
    "/home/example/Applications/MoreCodex.AppImage",
  );
  assert.match(
    entry,
    /^Exec="\/home\/example\/Applications\/MoreCodex\.AppImage" --hidden$/m,
  );
...
test("Linux autostart escapes desktop-entry field codes in executable paths", () => {
  const entry = linuxDesktopEntry(
    { getPath: () => "/tmp/transient-electron" },
    "/home/example/100% ready/MoreCodex.AppImage",
  );
  assert.match(entry, /"\/home\/example\/100%% ready\/MoreCodex\.AppImage" --hidden/);
});
...
test("launcher runtime validation accepts native Windows paths and a named pipe", () => {
  const descriptorPath = "C:\\Users\\Example\\AppData\\Local\\MoreCodex\\launcher-browser.json";
  const config = {
    version: 3,
    releaseVersion: "0.2.0",
    mode: "browser-only",
    ...
    storageStatePath: "C:\\Users\\Example\\AppData\\Local\\MoreCodex\\storage-state.json",
    ...
    autoApproveToolCalls: false,
    controlToken: "runtime-supervisor-control-token-0123456789abcdef",
    runtimeCommand: ["C:\\Users\\Example\\.morecodex\\runtime\\bun.exe"],
  };
...
test("launcher delegates long-lived tunnel supervision to native runtimes connect", () => {
  const config = launcherConfig("C:\\Users\\Example\\.morecodex\\runtime\\launcher-browser.json", {
    mode: "full",
    runtimeCommand: [
      "C:\\Users\\Example\\.morecodex\\versions\\0.2.0-win32-x64\\runtime\\bun.exe",
      "C:\\Users\\Example\\.morecodex\\versions\\0.2.0-win32-x64\\app\\cli.js",
    ],
    brokerSocketPath: "\\\\.\\pipe\\codex-chatgpt-web-example",
    tunnel: {
      binaryPath: "C:\\Users\\Example\\.morecodex\\bin\\tunnel-client.exe",
      tunnelId: "tunnel_0123456789abcdef0123456789abcdef",
      runtimeKeyFile: "C:\\Users\\Example\\.morecodex\\secrets\\tunnel-runtime.key",
      profileDir: "C:\\Users\\Example\\.morecodex\\tunnel\\profiles",
      profileName: "codex-chatgpt-web",
      alias: "codex-chatgpt-web",
    },
  });
  const invocation = {
    executable: "C:\\Program Files\\MoreCodex\\resources\\runtime\\bun.exe",
    args: [
      "C:\\Program Files\\MoreCodex\\resources\\runtime\\app\\cli.js",
      "mcp",
      "--broker-socket",
      config.brokerSocketPath,
    ],
    cwd: "C:\\Program Files\\MoreCodex\\resources\\runtime",
  };
...
test("tunnel readiness preserves a native managed process identity when one is reported", async () => {
  ...
  const config = launcherConfig(path.join(root, "launcher.json"), { tunnel: { alias: "codex-web-gpt", tunnelId: "tunnel-test" } });
  const invocation = supervisor.runtimeCommand(["mcp", "--contract", "native", "--broker-socket", config.brokerSocketPath]);
  const connectArgs = managedTunnelConnectArgs(config, invocation);
  supervisor.runTunnelCommand = async () => ({
    code: 0,
    output: JSON.stringify({
      entries: [{
        ...
        live_runtime: {
          ...
          status: { control_plane_tunnel_id: "tunnel-test", channels: [{ name: "main", details: [{ key: "command", value: connectArgs[connectArgs.indexOf("--mcp-command") + 1] }] }] },
        },
      }],
    }),
  });
  try {
    assert.deepEqual(await supervisor.readTunnelHealth(config), {
      ready: true,
      pid: 123_456_779,
      state: "ready",
      processRunning: true,
...
test("tunnel readiness accepts the official tmux status without inventing a PID", async () => {
  ...
  const config = launcherConfig(path.join(root, "launcher.json"), { tunnel: { alias: "codex-chatgpt-web", tunnelId: "tunnel-test" } });
  const invocation = supervisor.runtimeCommand(["mcp", "--contract", "native", "--broker-socket", config.brokerSocketPath]);
  const connectArgs = managedTunnelConnectArgs(config, invocation);
  supervisor.runTunnelCommand = async () => ({
    code: 0,
    output: JSON.stringify({
      entries: [{
        ...
        runtime_state: "ready",
        classification: "active_runtime",
        live_runtime: { found: true, base_url: "http://127.0.0.1:12345", status: { control_plane_tunnel_id: "tunnel-test", channels: [{ name: "main", details: [{ key: "command", value: connectArgs[connectArgs.indexOf("--mcp-command") + 1] }] }] } },
      }],
    }),
  });
  try {
    const health = await supervisor.readTunnelHealth(config);
    assert.equal(health.ready, true);
    assert.equal(health.pid, null);
    await supervisor.waitForTunnel(config, 1);
