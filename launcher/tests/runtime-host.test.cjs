...
test("launcher refuses a dead runtime without changing the Codex route", async () => {
  const fixture = bridgeFixture({ active: false });
  fixture.supervisor.ownedRuntimeReady = async () => false;
  await assert.rejects(fixture.host.connectBridgeRoute(), /not ready/);
  assert.deepEqual(fixture.calls, []);
});

test("launcher refuses to connect when independent recovery protection cannot start", async () => {
  const fixture = bridgeFixture({ active: false });
  fixture.host.protectRoute = async () => { throw Error("guardian unavailable"); };
  await assert.rejects(fixture.host.connectBridgeRoute(), /guardian unavailable/);
  assert.deepEqual(fixture.calls, []);
});

test("setup disconnects before stopping and captures the disconnected rollback checkpoint", async () => {
  const fixture = bridgeFixture({ active: true });
  fixture.supervisor.readConfig = fixture.supervisor.readSetupConfig = () => ({ mode: "browser-only", browserHost: "launcher" });
  fixture.host.captureSetupCheckpoint = () => { fixture.calls.push("checkpoint"); return []; };
  fixture.host.beforeRuntimeStop = async name => fixture.host.restoreBridgeRouteWithinOperation(name);
  fixture.host.protectRoute = async () => fixture.calls.push("guard:ready");
  const originalRun = fixture.host.run;
  fixture.host.run = async (name, args) => {
    if (args[0] === "setup" || args.join(" ") === "route install") {
      fixture.calls.push(args.join(" "));
      return { stdout: JSON.stringify({ active: true, installed: true, errors: [] }) };
    }
    return originalRun(name, args);
  };
  await fixture.host.runSetup("core-setup", ["setup", "--browser-only"], {});
  const calls = fixture.calls;
  assert.ok(calls.indexOf("route disconnect") < calls.indexOf("runtime:stop"));
  assert.ok(calls.lastIndexOf("checkpoint") > calls.indexOf("route disconnect"));
  assert.ok(calls.indexOf("guard:ready") < calls.indexOf("route install"));
});

test("setup commits the route only after runtime readiness and browser validation", async () => {
  const fixture = bridgeFixture({ active: false });
  fixture.supervisor.readConfig = fixture.supervisor.readSetupConfig = () => ({ mode: "browser-only", browserHost: "launcher" });
  fixture.host.captureSetupCheckpoint = () => [];
  fixture.host.validateIntegration = async () => fixture.calls.push("browser:validated");
  fixture.host.run = async (_name, args) => {
    fixture.calls.push(args.join(" "));
    return { stdout: JSON.stringify({ active: true, installed: true, errors: [] }) };
  };
  await fixture.host.runSetup("core-setup", ["setup", "--browser-only"], {});
  assert.deepEqual(fixture.calls, ["setup --browser-only --preflight-only", "runtime:stop",
    "setup --browser-only --defer-codex-route", "runtime:start", "browser:validated", "route install"]);
});
...
test("bridge connection rejects a route command that did not reach the requested state", async () => {
  ...
  assert.deepEqual(fixture.calls, ["route status", "route connect", "route status", "runtime:stop"]);
});
...
test("failed first-time setup removes its route before restoring the unconfigured state", async () => {
  ...
  try {
    ...
    assert.deepEqual(calls.map((args) => args.join(" ")), [
      ...
      "setup --browser-only --defer-codex-route",
    ]);
...
test("launcher delegates an existing terminal-managed installation to the migration-aware CLI", async () => {
  ...
  host.run = async (_name, args) => {
    ...
    return { code: 0, stdout: JSON.stringify({ active: true, installed: true, errors: [] }), stderr: "" };
  };
...
test("failed launcher update restores every mutable setup file before restarting the previous runtime", async (t) => {
  ...
  try { fs.symlinkSync(sharedConfigPath, codexConfigPath); } catch (error) {
...
test("failed terminal migration restores removed launchd ownership before verifying the old runtime", async () => {
  ...
  const configPath = path.join(coreHome, "config.json");
  const daemonPlist = path.join(launchAgentsDir, "io.github.morecodex.daemon.plist");
  const tunnelPlist = path.join(launchAgentsDir, "io.github.morecodex.tunnel.plist");
  const oldConfig = {
    mode: "full",
