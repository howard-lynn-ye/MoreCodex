...
test("launcher publishes native packages for all supported desktop operating systems", () => {
  assert.equal(manifest.build.appId, "io.github.howard-lynn-ye.morecodex");
  assert.equal(manifest.build.artifactName, "morecodex-${version}-${os}-${arch}.${ext}");
...
test("release installers resolve checksummed native launcher assets", () => {
  ...
  assert.match(windowsInstaller, /morecodex-\$Version-win-\$Arch\.exe/);
  ...
  assert.equal(fullyQualifiedWindowsPath.test("C:\\Users\\tester\\MoreCodex"), true);
  assert.equal(fullyQualifiedWindowsPath.test("\\\\server\\share\\MoreCodex"), true);
  assert.equal(fullyQualifiedWindowsPath.test("C:MoreCodex"), false);
  assert.equal(fullyQualifiedWindowsPath.test("\\MoreCodex"), false);
  assert.equal(fullyQualifiedWindowsPath.test("MoreCodex"), false);
...
test("CI packages and smoke-launches on macOS, Windows, and Linux", () => {
  ...
  assert.match(release, /MoreCodex\.app/);
...
test("Linux AppImage fallback uses one owned extraction and removes it on exit", {
  ...
  const appImage = path.join(root, "MoreCodex.AppImage");
