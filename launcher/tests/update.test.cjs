...
test("Linux auto-update fails closed without the stable installer wrapper", () => {
  ...
  process.env.CODEX_WEB_GPT_APPIMAGE = "/opt/codex/MoreCodex.AppImage";
...
test("release comparison and platform assets are strict", () => {
  ...
  assert.equal(releaseAssetName("1.2.0", "darwin", "arm64"), "morecodex-1.2.0-mac-arm64.zip");
  assert.equal(releaseAssetName("1.2.0", "darwin", "x64"), "morecodex-1.2.0-mac-x64.zip");
  assert.equal(releaseAssetName("1.2.0", "win32", "x64"), "morecodex-1.2.0-win-x64.exe");
  assert.equal(releaseAssetName("1.2.0", "linux", "x64"), "morecodex-1.2.0-linux-x64.AppImage");
...
test("checksums and release URLs bind the exact expected asset", () => {
  ...
  assert.equal(
    validateReleaseAssetUrl(
      "https://github.com/howard-lynn-ye/MoreCodex/releases/download/v1.2.0/launcher.zip",
      "1.2.0",
      "launcher.zip",
    ),
    "https://github.com/howard-lynn-ye/MoreCodex/releases/download/v1.2.0/launcher.zip",
...
test("macOS bundle resolution never guesses outside Contents/MacOS", () => {
  assert.equal(
    macApplicationPath("/Applications/MoreCodex.app/Contents/MacOS/MoreCodex"),
    "/Applications/MoreCodex.app",
  );
  assert.throws(() => macApplicationPath("/tmp/MoreCodex"), /Could not resolve/);
});
    ...
    dependencies: {
      fetchRelease: async () => {
        ...
        return {
          ...
          assets: [
            {
              name: "morecodex-1.2.0-linux-x64.AppImage",
              browser_download_url: "https://github.com/howard-lynn-ye/MoreCodex/releases/download/v1.2.0/morecodex-1.2.0-linux-x64.AppImage",
            },
            {
              name: "checksums.txt",
              browser_download_url: "https://github.com/howard-lynn-ye/MoreCodex/releases/download/v1.2.0/checksums.txt",
            },
          ],
        };
      },
...
test("verified update is handed to one detached worker", async () => {
  ...
  const oldAppImage = path.join(root, "versions", "1.1.4", "MoreCodex.AppImage");
  ...
  try {
    const controller = createUpdateController({
      ...
      dependencies: {
        ...
        downloadText: async () => `${hash}  morecodex-1.2.0-linux-x64.AppImage\n`,
...
test("detached worker replaces an installed Linux AppImage and removes the old version", {
  ...
  const oldTarget = path.join(versionsRoot, "1.1.4", "MoreCodex.AppImage");
  const newTarget = path.join(versionsRoot, "1.2.0", "MoreCodex.AppImage");
  const wrapper = path.join(root, "bin", "codex-web-gpt");
  ...
  try {
    ...
    assert.match(fs.readFileSync(wrapper, "utf8"), /versions\/1\.2\.0\/MoreCodex\.AppImage/);
