...
test("security challenges never trigger automatic refresh, including after unrelated success", async () => {
  const calls = [];
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    turnTabs: new Map(),
  ...
  assert.ok(calls.some(([name, state]) => name === "setState" && /Automatic refresh is paused/.test(state.message)));
  ...
  assert.equal(fixture.cloudflareChallengeRecoveryArmed, false);
  BrowserHost.prototype.handleChatGptBackendResponse.call(fixture, challenge);
  assert.deepEqual(calls.filter(([name]) => name === "loadURL"), []);
});
...
test("hidden startup gives the primary browser a viewport and clears emulation when shown", () => {
  const calls = [];
  const hidden = { x: 1201, y: 801, width: 1200, height: 800 };
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    primaryRendererReady: true, primaryViewportDirty: true,
    bounds: { x: 0, y: 0, width: 900, height: 650 },
    hiddenTurnBounds: () => hidden,
    getBrowserInteractionMode: () => "automatic",
    view: { setBounds: () => {}, setVisible: () => {}, webContents: {
      enableDeviceEmulation: value => calls.push(value.viewSize),
      disableDeviceEmulation: () => calls.push("disabled"),
    } },
  });
  fixture.presentPrimaryView(false);
  fixture.presentPrimaryView(false);
  fixture.presentPrimaryView(true);
  assert.deepEqual(calls, [{ width: 1200, height: 800 }, "disabled"]);
});
...
test("complete server authentication persists even before the composer loads", async () => {
  const fixture = {
    ...
    view: {
      webContents: {
        ...
        session: { flushStorageData() {}, cookies: { async flushStore() {} } },
      },
    },
    setState(patch) { this.state = { ...this.state, ...patch }; },
    snapshot() { return { ...this.state }; },
...
test("explicit login waits for an in-flight saved-session refresh before taking browser ownership", async () => {
  ...
  finishRefresh();
  await login;
  assert.deepEqual(calls, ["ChatGPT login", "probe", "persist"]);
});
...
test("concurrent authentication probes share one request and a failed network check preserves the saved login", async () => {
  let finish;
  let requests = 0;
  const fixture = {
    state: { authenticated: true },
    view: { webContents: {
      isDestroyed: () => false,
      getURL: () => "https://chatgpt.com/?temporary-chat=true",
      executeJavaScript: () => { requests++; return new Promise(resolve => { finish = resolve; }); },
      loadURL: () => assert.fail("Network failure must not navigate"),
    } },
    setState(patch) { this.state = { ...this.state, ...patch }; },
    snapshot() { return this.state; },
  };
  ...
  assert.equal(first, second);
  finish({ sessionAuthenticated: false, sessionStatus: "unavailable", readyState: "complete" });
  const result = await first;
  assert.equal(requests, 1);
  assert.equal(result.authenticated, true);
  assert.equal(result.status, "error");
  assert.match(result.message, /Saved login was not cleared/);
});

test("authentication popup closing cannot reload a completed login", () => {
  const view = { webContents: { isDestroyed: () => false, close() {} } };
  const fixture = {
    authView: view, manualOperation: "ChatGPT login",
    window: { contentView: { removeChildView() {} } },
    view: { webContents: { isDestroyed: () => false, loadURL: () => assert.fail("Unexpected refresh") } },
    syncViewVisibility() {}, logger: { info() {} },
  };
  BrowserHost.prototype.closeAuthView.call(fixture, view, true, true);
  assert.equal(fixture.authView, null);
});

test("authentication rejects a supplied popup from another account session", () => {
  const fixture = { view: { webContents: { session: {} } }, closeAuthView() {} };
  assert.throws(() => BrowserHost.prototype.createAuthView.call(fixture, { webContents: { session: {} } }), /persistent session/);
});

test("OAuth completion in the shared session succeeds without destroying or reloading either page", async () => {
  ...
  const fixture = {
    ...
    view: {
      webContents: {
        ...
        session: { flushStorageData() {}, cookies: { async flushStore() {} } },
        loadURL: async (url) => {
          assert.fail(`Unexpected login refresh: ${url}`);
        },
      },
    },
    closeAuthView(view, closeContents, refreshMain) {
  ...
  const result = await BrowserHost.prototype.probeAuthentication.call(fixture);
  assert.equal(result.authenticated, true);
  assert.equal(fixture.authView, completedAuthView);
  assert.equal(primaryReady, false);
});

test("a successful primary login redirect preserves its current document", async () => {
...
test("an authenticated primary surface does not change the popup during a probe", async () => {
  const staleAuthView = {
    webContents: {
      isDestroyed: () => false,
      executeJavaScript: async () => ({
  ...
  const result = await BrowserHost.prototype.probeAuthentication.call(fixture);
  assert.equal(result.authenticated, true);
  assert.equal(fixture.authView, staleAuthView);
