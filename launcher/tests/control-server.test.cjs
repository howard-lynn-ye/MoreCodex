...
test("account host status authenticates the descriptor owner without exposing browser contents", async () => {
  const server = await new BrowserControlServer({
    logger: { info() {}, error() {} }, getPreferences: () => ({}),
    getBrowserHost: () => ({ profile: "production", partition: "persist:codex-web-gpt-chatgpt",
      state: { authenticated: true, status: "ready", url: "private-url", title: "private-title" },
      currentOperation: () => null, turnTabs: new Map() }),
  }).start();
  const d = server.descriptor();
  try {
    assert.equal((await fetch(`${d.endpoint}/v1/account-host/status`)).status, 401);
    const response = await fetch(`${d.endpoint}/v1/account-host/status`, { headers: { authorization: `Bearer ${d.token}` } });
    assert.equal(response.status, 200);
    const result = await response.json();
    assert.deepEqual(result, { pid: process.pid, profile: "production", partition: "persist:codex-web-gpt-chatgpt", authenticated: true, status: "ready", operation: null, activeTurns: 0 });
  } finally { await server.close(); }
});

test("account host controls require authentication and refuse stopping an active account", async () => {
  let busy = true, stopped = 0, opened = 0;
  const server = await new BrowserControlServer({
    logger: { info() {}, warn() {}, error() {} },
    getBrowserHost: () => ({ currentOperation: () => busy ? "turn" : null, turnTabs: new Map() }),
    getPreferences: () => ({}),
    onAccountHostLogin: async () => { opened += 1; },
    onAccountHostStop: async () => { stopped += 1; },
  }).start();
  const { endpoint, token } = server.descriptor();
  const request = (action, authorized = true) => fetch(`${endpoint}/v1/account-host/${action}`, {
    method: "POST", headers: authorized ? { authorization: `Bearer ${token}` } : {},
  });
  try {
    assert.equal((await request("stop", false)).status, 401);
    assert.equal((await request("stop")).status, 409); assert.equal(stopped, 0);
    assert.equal((await request("login")).status, 200); assert.equal(opened, 1);
    busy = false; assert.equal((await request("stop")).status, 202);
