...
class BrowserHost {
  ...
  bindWebContents() {
    ...
    contents.on("did-finish-load", () => {
      ...
      this.primaryViewportDirty = true;
      this.syncViewVisibility();
      if (this.manualOperation === "ChatGPT login") {
  ...
  async refreshChatGptHomeDocument() {
    // Reuse an existing document. Authentication/capability checks must not reload a live login.
  ...
  handleChatGptBackendResponse(details) {
    ...
    // A successful unrelated request cannot authorize another reload of the login document.
    // Leave the session untouched and let the user complete the site's security check once.
    this.setState({ status: "error", message: "ChatGPT security check requires attention in this account's browser. Automatic refresh is paused; other accounts are unaffected.", loading: false });
    return true;
  }
  ...
  presentPrimaryView(visible) {
    ...
    if (this.primaryRendererReady && browserInteractionModeFor(this) === "automatic") {
      if (visible && this.primaryViewport) {
        this.view.webContents.disableDeviceEmulation();
        this.primaryViewport = null;
      } else if (!visible && (this.primaryViewportDirty
        || this.primaryViewport?.width !== bounds.width || this.primaryViewport?.height !== bounds.height)) {
        this.enableHiddenTurnViewport(this.view.webContents, bounds);
        this.primaryViewport = { width: bounds.width, height: bounds.height };
      }
      this.primaryViewportDirty = false;
    }
    this.view.setVisible(true);
  }
  ...
  createAuthView(options = {}, requestedUrl = "") {
    ...
    const ownedSession = this.view.webContents.session;
    if (options.webContents && options.webContents.session !== ownedSession) {
      throw new Error("Authentication popup does not belong to this account's persistent session");
    }
  ...
  closeAuthView(authView, closeContents, _refreshMain = false) {
  ...
  probeAuthentication() {
    if (this.authenticationProbeOperation) return this.authenticationProbeOperation;
    const operation = BrowserHost.prototype.readAuthentication.call(this);
    const tracked = operation.finally(() => {
      if (this.authenticationProbeOperation === tracked) this.authenticationProbeOperation = null;
    });
    this.authenticationProbeOperation = tracked;
    return tracked;
  }
  ...
  async readAuthentication() {
    ...
    if (!url.startsWith(CHATGPT_ORIGIN)) {
      this.setState({ status: "loading", message: "Waiting for this account's authentication page", url });
      return this.snapshot();
    }
    const probe = (contents) => contents.executeJavaScript(`(async () => {
      ...
      if (new URL(initialSurface.url).origin === expectedUrl.origin) {
        ...
        try {
          ...
          sessionStatus = sessionAuthenticated ? "authenticated"
            : payload !== null || response.status === 401 ? "signed-out" : "unavailable";
        } catch {}
        finally { clearTimeout(timeout); }
      }
      return { ...readSurface(), sessionAuthenticated, sessionStatus };
    })()`, true).catch(() => ({
      url: "",
      composer: false,
      temporary: false,
    ...
    if (!result.sessionAuthenticated
      ...
      if (authResult.sessionAuthenticated) {
        // Both views use the same persistent session. Keep the completed page intact;
        // neither closing the popup nor reloading the primary is an authentication step.
        result = authResult;
      }
    }
    if (result.sessionAuthenticated) {
      ...
      if (!wasAuthenticated) {
        // Persist as soon as authentication is proved, not only on graceful shutdown.
        await BrowserHost.prototype.persistSession.call(this);
        this.logger.info("browser.authenticated", { origin: CHATGPT_ORIGIN, persistentPartition: this.partition });
      }
    } else if (result.sessionStatus === "unavailable") {
      this.setState({ status: "error", message: "Cannot check this account's ChatGPT session. Check the connection or security check, then retry. Saved login was not cleared.", url: result.url || url, loading: false });
    } else {
  ...
  async smokeTest(options = {}) {
    ...
    return await this.withManualOperation("browser smoke test", () => this.runSmokeTest(options));
  }
  ...
  async runSmokeTest({ background = false } = {}) {
    requireAutomaticBrowserInspection(this, "ChatGPT browser smoke test");
    const connectorName = this.connectorName();
    if (background) {
      await this.ready();
      this.activateHomeSurface();
      this.presentPrimaryView(false);
    } else {
      this.show();
