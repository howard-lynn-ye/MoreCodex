...
class RuntimeSupervisor {
  ...
  tryWriteState(status, detail) {
    if (["degraded", "failed"].includes(status) && !this.stopping && this.onUnavailable && !this.routeRecovery) {
      this.routeRecovery = Promise.resolve().then(() => this.onUnavailable()).catch(error => {
        this.logger.error("runtime.route_recovery_failed", { message: errorMessage(error) });
      }).finally(() => { this.routeRecovery = null; });
    }
    try {
  ...
  async readTunnelHealth(config) {
    ...
    try {
      ...
      let liveRuntime = entry.live_runtime && typeof entry.live_runtime === "object"
        ? entry.live_runtime
        : {};
      // The native inventory may associate an alias with ANY running client for the same
      // remote tunnel ID. Readiness alone therefore does not identify this launcher's broker.
      if (["healthy", "ready"].includes(runtimeState)) {
        const invocation = this.runtimeCommand([
          "mcp", "--contract", config.browserInteractionMode === "manual" ? "safe" : "native",
          "--broker-socket", config.brokerSocketPath,
        ]);
        const expectedCommand = managedTunnelMcpCommand(invocation);
        const matches = candidate => {
          const channel = candidate?.status?.channels?.find(value => value?.name === "main");
          const actualCommand = channel?.details?.find(value => value?.key === "command")?.value;
          return candidate?.found === true
            && candidate.status?.control_plane_tunnel_id === tunnel.tunnelId
            && actualCommand === expectedCommand;
        };
        // The inventory can place a shared runtime's live details under another alias.
        // Only the exact command/broker binding, never the other alias's ready flag, may match it.
        const verified = matches(liveRuntime) ? liveRuntime
          : parsed.entries.map(candidate => candidate?.live_runtime).find(matches);
        if (!verified) {
          this.tunnelHealthBaseUrl = null;
          return {
            ready: false, pid: null, state: "conflict", processRunning: undefined,
            healthy: false, absent: false, statusKnown: true, ownershipConflict: true,
            detail: "Tunnel runtime ownership could not be verified: the live MCP command or tunnel ID"
              + " does not match this launcher. Stop the other installation's runtime before retrying.",
          };
        }
        liveRuntime = verified;
      }
  ...
  async observeTunnelForMonitor(config) {
    ...
    try {
      ...
      if (inventory.ownershipConflict) return { ...inventory, fatal: true };
  ...
  async waitForTunnel(
    ...
    while (Date.now() < deadline) {
      ...
      if (health.ownershipConflict) throw new Error(health.detail);
      if (health.pid) {
        this.tunnel = {
          pid: health.pid,
          exitCode: null,
  ...
  async startTunnel(config, operationName = "runtime-start", { forceRestart = false } = {}) {
    ...
    try {
      ...
      // A conflict is another installation, not a failed child we are entitled to stop.
  ...
  async startDaemon(config) {
    ...
    let child;
    try {
      try { await this.waitForPortRelease(config, 500); }
      catch {
        throw new Error(`Responses port ${config.host}:${config.port} is already occupied. Close its other service or choose a different bridge port.`);
      }
