...
class BrowserControlServer {
  constructor({ logger, getBrowserHost, getPreferences, onAccountHostStop, onAccountHostLogin }) {
    this.logger = logger;
    this.getBrowserHost = getBrowserHost;
    this.getPreferences = getPreferences;
    this.onAccountHostStop = onAccountHostStop;
    this.onAccountHostLogin = onAccountHostLogin;
  ...
  async handle(request, response) {
    ...
    if (request.method === "POST" && request.url === "/v1/account-host/login" && this.onAccountHostLogin) {
      await this.onAccountHostLogin();
      writeJson(response, 200, { ok: true });
      return;
    }
    if (request.method === "GET" && request.url === "/v1/account-host/status") {
      const host = this.getBrowserHost();
      if (!host) { writeJson(response, 503, { error: "host_starting" }); return; }
      writeJson(response, 200, {
        pid: process.pid, profile: host.profile, partition: host.partition,
        authenticated: host.state?.authenticated === true, status: host.state?.status,
        operation: host.currentOperation(),
        activeTurns: [...(host.turnTabs?.values() ?? [])].filter(tab => tab.status === "running").length,
      });
      return;
    }
    if (request.method === "POST" && request.url === "/v1/account-host/stop" && this.onAccountHostStop) {
      const host = this.getBrowserHost();
      if (host?.currentOperation() || [...(host?.turnTabs?.values() ?? [])].some(tab => tab.status === "running")) {
        writeJson(response, 409, { error: "account_busy", message: "Finish or interrupt this account's active turn before stopping it" });
        return;
      }
      writeJson(response, 202, { ok: true });
      setImmediate(() => { void this.onAccountHostStop(); });
      return;
    }
