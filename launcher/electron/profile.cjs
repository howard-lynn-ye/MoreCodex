...
function resolveLauncherProfile({
  ...
  if (!development) {
    ...
    env = { ...env,
      CODEX_CHATGPT_WEB_HOME: option("--core-home", env.CODEX_CHATGPT_WEB_HOME),
      CODEX_HOME: option("--codex-home", env.CODEX_HOME),
      CODEX_WEB_GPT_LAUNCHER_DATA_DIR: option("--launcher-data", env.CODEX_WEB_GPT_LAUNCHER_DATA_DIR),
    };
    const coreHome = env.CODEX_CHATGPT_WEB_HOME?.trim()
      ? resolveUserPath(env.CODEX_CHATGPT_WEB_HOME.trim(), homeDir)
      : path.join(homeDir, ".morecodex");
    const userData = env.CODEX_WEB_GPT_LAUNCHER_DATA_DIR?.trim()
      ? resolveUserPath(env.CODEX_WEB_GPT_LAUNCHER_DATA_DIR.trim(), homeDir)
      : path.join(appData, "MoreCodex");
    return {
      kind: PRODUCTION_PROFILE,
      displayName: "MoreCodex",
      coreHome,
  ...
  const coreHome = env.CODEX_WEB_GPT_DEV_HOME?.trim()
    ...
    : path.join(homeDir, ".morecodex-dev");
