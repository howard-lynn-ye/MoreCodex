...
const REPOSITORY = "howard-lynn-ye/MoreCodex";
const RELEASE_API_URL = `https://api.github.com/repos/${REPOSITORY}/releases/latest`;
const USER_AGENT = "morecodex-launcher-updater";
...
function releaseAssetName(version, platform = process.platform, arch = process.arch) {
  if (platform === "darwin" && ["arm64", "x64"].includes(arch)) {
    return `morecodex-${version}-mac-${arch}.zip`;
  }
  if (platform === "win32" && arch === "x64") {
    return `morecodex-${version}-win-x64.exe`;
  }
  if (platform === "linux" && arch === "x64") {
    return `morecodex-${version}-linux-x64.AppImage`;
  }
  return null;
}
...
function findMacApplication(root) {
  ...
  const executable = path.join(application, "Contents", "MacOS", "MoreCodex");
...
function createUpdateController({
  ...
  async function beginInstall() {
    ...
    pending = (async () => {
      ...
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "morecodex-update-"));
      try {
