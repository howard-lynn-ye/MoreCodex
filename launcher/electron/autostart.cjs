...
const LINUX_DESKTOP_NAME = "io.github.howard-lynn-ye.morecodex.desktop";
...
Version=1.0
Name=MoreCodex
Comment=Start the MoreCodex launcher in the background
...
function windowsLoginArgs(app, env = process.env) {
  const args = ["--hidden"];
  for (const [flag, value] of [["--core-home", env.CODEX_CHATGPT_WEB_HOME],
    ["--codex-home", env.CODEX_HOME], ["--launcher-data", app.getPath("userData")]]) {
...
function setAutostart(app, enabled) {
  ...
  if (process.platform === "darwin" || process.platform === "win32") {
    const args = process.platform === "win32" ? windowsLoginArgs(app) : ["--hidden"];
...
function getAutostart(app) {
  ...
  if (process.platform === "darwin" || process.platform === "win32") {
    return {
      supported: true,
      enabled: app.getLoginItemSettings({ args: process.platform === "win32" ? windowsLoginArgs(app) : ["--hidden"] }).openAtLogin === true,
    };
  }
  return { supported: false, enabled: false };
}
