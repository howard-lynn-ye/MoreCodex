...
const { serviceOnly, disconnectBeforeShutdown } = require("./integration-safety.cjs");
const { DEVELOPMENT_PROFILE, resolveLauncherProfile } = require("./profile.cjs");
const { runtimeBundlePaths, explicitRuntimeBundle } = require("./runtime-command.cjs");
...
if (SERVICE_ONLY) LAUNCHER_PROFILE.displayName += " (Service validation)";
const accountLabelIndex = process.argv.indexOf("--account-label");
if (accountLabelIndex >= 0) {
  const label = process.argv[accountLabelIndex + 1];
  if (!label || !/^[A-Za-z0-9][A-Za-z0-9 _.-]{0,39}$/.test(label)) throw new Error("Invalid account label");
  LAUNCHER_PROFILE.displayName += ` — ${label}`;
}
const CORE_HOME = LAUNCHER_PROFILE.coreHome;
const BROWSER_DESCRIPTOR_PATH = path.join(CORE_HOME, "runtime", "launcher-browser.json");
const EXPLICIT_RUNTIME_BUNDLE = explicitRuntimeBundle();
if (IS_DEV_PROFILE && EXPLICIT_RUNTIME_BUNDLE) throw new Error("A production runtime deployment cannot be used with the DEV profile");
const BROWSER_HELPER_PATH = EXPLICIT_RUNTIME_BUNDLE?.helperScript || (app.isPackaged
  ? path.join(process.resourcesPath, "runtime", "app", "browser-helper.cjs")
  : path.join(SOURCE_ROOT, ".launcher-runtime", "browser-helper.cjs"));
const GITHUB_URL = "https://github.com/howard-lynn-ye/MoreCodex";
...
if (process.platform === "win32") {
  app.setAppUserModelId(IS_DEV_PROFILE ? "io.github.howard-lynn-ye.morecodex.dev" : "io.github.howard-lynn-ye.morecodex");
}
...
async function restoreCodexRouteAfterRuntimeFailure({ logger, stateStore }) {
  if (IS_DEV_PROFILE || SERVICE_ONLY) return { restored: false };
  try {
...
const NATIVE_COPY = Object.freeze({
  en: Object.freeze({
    openLauncher: "Open MoreCodex",
    quit: "Quit",
    exportDiagnostics: "Export privacy-safe diagnostics",
    cancel: "Cancel",
    remove: "Remove",
    removeTitle: "Remove MoreCodex",
...
async function requestQuit() {
  ...
  try {
    ...
    if (activeOperation) {
      throw new Error(`Wait for ${activeOperation} to finish before quitting MoreCodex`);
    }
    if (!INSPECT_ONLY) await disconnectBeforeShutdown({ host: runtimeHost, supervisor: runtimeSupervisor,
      isolated: IS_DEV_PROFILE || SERVICE_ONLY });
    stopCatalogVerificationMonitor();
    quitting = true;
...
async function start() {
  ...
  app.on("second-instance", (_event, argv) => {
    // Concurrent background starts share the existing profile without stealing focus.
  ...
  if (!IS_DEV_PROFILE && !SERVICE_ONLY
  ...
  browserControl = await new BrowserControlServer({
    ...
    ...(INSPECT_ONLY ? {
      onAccountHostStop: () => requestQuit(),
      onAccountHostLogin: async () => {
        showMainWindow();
        // The renderer owns the browser slot and its bounds. Revealing the WebContents
        // alone leaves it hidden behind Setup when login is requested through the CLI.
        send("launcher:account-login-requested");
        // Acknowledge opening the surface immediately; website navigation can take longer
        // than the control client's timeout. Its failure must never replay a login request.
        void browserHost.reveal(false).catch(() => {
          logger.warn("browser.account_login_navigation_failed");
          browserHost.setState({ status: "error", message: "This account's login page could not load. Check its browser and retry manually.", loading: false });
        });
      },
    } : {}),
  }).start();
  ...
  // Keep the profile/account visible in the native title instead of the renderer's generic title.
  mainWindow.on("page-title-updated", event => event.preventDefault());
  runtimeSupervisor.onUnavailable = async () => {
    const result = await restoreCodexRouteAfterRuntimeFailure({ logger, stateStore });
    if (result.error) throw new Error(result.error);
  };
  if (!IS_DEV_PROFILE && !SERVICE_ONLY && !INSPECT_ONLY) {
    const { WindowsRouteGuard } = require("./route-guard.cjs");
    const guard = process.platform === "win32" ? new WindowsRouteGuard({
      coreHome: CORE_HOME, codexHome: LAUNCHER_PROFILE.codexHome,
      invocation: runtimeHost.command([]),
      onRestored: status => {
        logger.warn("bridge.independent_route_recovery", { state: status.state, reason: status.reason });
        const state = stateStore.update({ codexCatalogVerified: false, codexRestartRequired: true });
        send("launcher:state-changed", state);
        publishOperation({ name: "route-recovery", status: status.state === "restored" ? "succeeded" : "failed",
          message: status.state === "restored"
            ? "The bridge became unavailable. The previous Codex route was restored; restart Codex."
            : "Automatic route restoration failed. Open diagnostics before restarting Codex." });
      },
    }) : null;
    runtimeHost.protectRoute = async () => guard?.arm(runtimeSupervisor.readConfig());
    runtimeHost.verifyRouteProtection = () => guard?.assertReady();
    runtimeHost.releaseRouteProtection = async () => guard?.stop();
    if (guard) guard.onUnavailable = async () => {
      const result = await restoreCodexRouteAfterRuntimeFailure({ logger, stateStore });
      if (result.error) throw new Error(result.error);
    };
    runtimeHost.beforeRuntimeStop = async name => {
      await runtimeHost.restoreBridgeRouteWithinOperation(name);
      await runtimeHost.releaseRouteProtection();
    };
    // Recover a stale route before browser login, runtime upgrades or startup.
    // Reconnection happens only after runtime and browser validation below.
    await runtimeHost.restoreBridgeRoute("startup-route-recovery");
  }
  ...
  runtimeHost.validateIntegration = async () => {
    if (stateStore.read().browserInteractionMode === "automatic") {
      await browserHost.smokeTest({ background: true });
      if (runtimeSupervisor.readConfig()?.mode === "full") {
        await browserHost.verifyConnector(runtimeHost.mcpConnectorName());
      }
    }
  };
  ...
  if (!launcherSmokeTest && !INSPECT_ONLY && stateStore.read().browserInteractionMode === "automatic") {
  ...
  if (IS_DEV_PROFILE) {
  ...
  } else if (!INSPECT_ONLY) void (async () => {
    await startupAuthenticationRefresh;
    const upgrade = SERVICE_ONLY ? { updated: false } : await runtimeHost.upgradeManagedRuntime();
    if (upgrade.updated) {
      const state = stateStore.update({
        coreSetupComplete: true,
        codexCatalogVerified: false,
    ...
    if (SERVICE_ONLY) return { ...runtime, bridgeRouteChanged: false };
    ...
    if (runtime.status === "external" || runtime.status === "needs-setup") {
      const detail = runtime.detail || (
        runtime.status === "external"
          ? "Another process owns the configured MoreCodex runtime"
...
void start().catch((error) => {
  ...
  try {
    dialog.showErrorBox("MoreCodex could not start", message);
  } catch {}
  app.exit(1);
});
