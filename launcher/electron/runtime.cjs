...
class RuntimeHost {
  ...
  captureSetupCheckpoint(snapshot) {
    ...
    if (snapshot.owner === "external" && this.platform === "darwin") {
      paths.add(path.join(this.launchAgentsDir, "io.github.morecodex.daemon.plist"));
      paths.add(path.join(this.launchAgentsDir, "io.github.morecodex.tunnel.plist"));
    }
    const tunnels = [
  ...
  async run(name, args, options = {}) {
    require("./integration-safety.cjs").assertServiceOnlyCommand(args);
  ...
  async connectBridgeRoute() {
    ...
    try {
      ...
      if (!config || !await this.supervisor.ownedRuntimeReady(config)) {
        throw new Error("Cannot connect Codex: the owned Responses runtime and required Tunnel are not ready");
      }
      await this.validateIntegration?.();
      await this.protectRoute?.();
      ...
      try {
        ...
        if (!await this.supervisor.ownedRuntimeReady(config)) throw new Error("Runtime was lost while connecting Codex");
        await this.verifyRouteProtection?.();
        return result;
      } catch (error) {
        // A failed verification can follow a successful config write. Keep the
        // service alive if rollback cannot be proved; never strand that route.
  ...
  async uninstallIntegration() {
    ...
    try {
      try {
        await this.beforeRuntimeStop?.(name);
      ...
      try {
        const result = await this.run(name, ["uninstall", "--yes", "--launcher-control"], {
          ...
          env: this.launcherControlEnvironment(),
          message: "Restoring the previous Codex route",
          successMessage: "MoreCodex integration removed",
  ...
  async runSetup(name, args, options) {
    ...
    try {
      ...
      if (this.launcherProfile === "production" && this.beforeRuntimeStop) {
        await this.beforeRuntimeStop(name);
        // A rollback must restore the usable, disconnected baseline, even when
        // an older installation was connected before the transition began.
      ...
      const stagedArgs = this.launcherProfile === "production" ? [...args, "--defer-codex-route"] : args;
      const result = await this.run(name, stagedArgs, options);
      const runtime = await this.supervisor.startIfConfigured();
      ...
      if (this.launcherProfile === "production") {
        await this.validateIntegration?.();
        await this.protectRoute?.();
        const installed = await this.run(name, ["route", "install"], { embedded: true,
          message: "Committing the verified Codex route", timeoutMs: 15000 });
        parseBridgeRouteResult(installed.stdout, { expectedActive: true });
        await this.verifyRouteProtection?.();
      }
      return result;
    } catch (error) {
