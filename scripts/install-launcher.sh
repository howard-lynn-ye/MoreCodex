...
REPOSITORY="${CODEX_WEB_GPT_REPOSITORY:-howard-lynn-ye/MoreCodex}"
...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest MoreCodex release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="morecodex-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
BASE_URL="https://github.com/$REPOSITORY/releases/download/v$VERSION"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/morecodex-launcher.XXXXXX")"
...
if [ "$OS" = "Darwin" ]; then
  ...
  SOURCE_APP="$STAGE_DIR/MoreCodex.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/MoreCodex" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/MoreCodex.app"
  if pgrep -x "MoreCodex" >/dev/null 2>&1; then
    echo "Quit MoreCodex before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/MoreCodex.previous.app"
...
TARGET="$TARGET_DIR/MoreCodex.AppImage"
WRAPPER="$BIN_DIR/codex-web-gpt"
CORE_HOME="${CODEX_CHATGPT_WEB_HOME:-$HOME/.morecodex}"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "MoreCodex\\.AppImage" >/dev/null 2>&1; then
  echo "Quit MoreCodex before updating it" >&2
  exit 1
fi
