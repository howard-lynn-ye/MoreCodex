...
$Repository = if ($env:CODEX_WEB_GPT_REPOSITORY) { $env:CODEX_WEB_GPT_REPOSITORY } else { "howard-lynn-ye/MoreCodex" }
...
if (-not $Version) { throw "Could not resolve the latest MoreCodex release" }
...
$Asset = "morecodex-$Version-win-$Arch.exe"
$BaseUrl = "https://github.com/$Repository/releases/download/v$Version"
$Temp = Join-Path ([System.IO.Path]::GetTempPath()) "morecodex-$([guid]::NewGuid().ToString('N'))"
New-Item -ItemType Directory -Path $Temp | Out-Null
try {
  if (Get-Process -Name "MoreCodex" -ErrorAction SilentlyContinue) {
    throw "Quit MoreCodex before updating it"
  }
  ...
  $InstallRegistry = "HKCU:\Software\f3a25aab-fc95-4f2d-9fc2-345abce01694"
  ...
  $Executable = Join-Path $InstallLocation "MoreCodex.exe"
