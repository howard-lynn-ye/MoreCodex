...
test("a non-retryable streamed failure cannot restart the browser on reconnect; a new turn can run", async () => {
  const config = defaultConfig("browser-only");
  const body = (turn: string) => ({ model: "chatgpt-web/high", stream: true,
    client_metadata: { "x-codex-turn-metadata": JSON.stringify({ thread_id: "terminal_failure_replay", turn_id: turn }) },
    input: [{ type: "message", role: "user", content: [{ type: "input_text", text: "terminal failure acceptance" }],
      internal_chat_message_metadata_passthrough: { turn_id: turn } }],
  });
  let runs = 0;
  const send = (turn: string) => responseRequest(new Request("http://127.0.0.1/v1/responses", {
    method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body(turn)),
  }), config, () => ({ name: "terminal-replay-test", async runTurn(_parsed, _incoming, emit) {
    runs++;
    emit({ type: "error", message: "Required compaction checkpoint was not submitted", status: 400,
      errorType: "invalid_request_error", code: "compaction_handoff_missing", retryable: false });
  } }));
  const first = await send("terminal_original");
  expect(first.headers.get("content-type")).toBe("text/event-stream");
  expect(await first.text()).toContain("compaction_handoff_missing");
  for (let attempt = 0; attempt < 3; attempt++) {
    const replay = await send("terminal_original");
    expect(replay.status).toBe(400);
    expect(await replay.json()).toMatchObject({ error: { code: "compaction_handoff_missing" } });
  }
  expect(runs).toBe(1);
  await (await send("terminal_new_user_turn")).text();
  expect(runs).toBe(2);
});
