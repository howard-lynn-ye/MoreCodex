...
test("active compaction returns current and later results across separate broker and client processes", async () => {
  const root = mkdtempSync(join(shortSocketTempRoot(), "cgw-compaction-process-"));
  const broker = TurnBroker.forSocket(defaultBrokerEndpoint(root));
  ...
  try {
    const token = await broker.register({
      ...
      tools: [{ name: "exec_command", description: "Run one command", parameters: { type: "object" } }],
    }, 30_000, "trace_cross_process_compaction");
    ...
    const clientCode = `
      ...
      const endpoint = process.env.CGW_TEST_BROKER_ENDPOINT;
      const { bindingId } = await callTurnBroker(endpoint, { method: "claim", token: process.env.CGW_TEST_BROKER_TOKEN });
      const current = await callTurnBroker(endpoint, { method: "invoke", bindingId, wireName: "exec_command", arguments: { cmd: "first" } });
      const later = await callTurnBroker(endpoint, { method: "invoke", bindingId, wireName: "exec_command", arguments: { cmd: "later" } });
      console.log(JSON.stringify({ current, later }));
    `;
    client = Bun.spawn({
      cmd: [process.execPath, "--eval", clientCode],
      env: { ...process.env, CGW_TEST_BROKER_ENDPOINT: broker.socketPath, CGW_TEST_BROKER_TOKEN: token },
      stdout: "pipe", stderr: "pipe",
    });
    const [invocation] = await broker.nextToolBatch(token);
    broker.requestCompaction(token, { content: [{ type: "text", text: "checkpoint instead" }], isError: true });
    broker.completeTool(token, invocation!.callId, { content: [{ type: "text", text: "current result" }] });
    if (!(client.stdout instanceof ReadableStream) || !(client.stderr instanceof ReadableStream)) {
      throw new Error("Cross-process broker fixture did not create output pipes");
    }
    const [exitCode, stdout, stderr] = await Promise.all([
      client.exited, new Response(client.stdout).text(), new Response(client.stderr).text(),
    ]);
    expect({ exitCode, stderr }).toEqual({ exitCode: 0, stderr: "" });
    expect(JSON.parse(stdout.trim().split(/\r?\n/).at(-1)!)).toEqual({
      current: { content: [{ type: "text", text: "current result" }] },
      later: { content: [{ type: "text", text: "checkpoint instead" }], isError: true },
    });
    expect(broker.compactionDeliveryCount(token)).toBe(1);
  } finally {
    if (client && client.exitCode === null) { client.kill(); await client.exited; }
    await broker.close();
    rmSync(root, { recursive: true, force: true });
  }
});

test.skipIf(process.platform !== "win32")("broker client retires its socket after a complete response without waiting for a peer FIN", async () => {
  const root = mkdtempSync(join(shortSocketTempRoot(), "cgw-frame-retirement-"));
  const endpoint = defaultBrokerEndpoint(root);
  const sockets = new Set<Socket>();
  let clientConnected = 0;
  let clientClosed = false;
  const originalConnect = Socket.prototype.connect;
  // Observe the actual client's close event without replacing the transport or its behavior.
  const observedConnect = mock.method(Socket.prototype, "connect", function (this: Socket, ...args: unknown[]) {
    clientConnected += 1;
    this.once("close", () => { clientClosed = true; });
  ...
  const server = createServer(socket => {
    ...
    socket.on("data", chunk => {
      ...
      const request = JSON.parse(received.slice(0, received.indexOf("\n")));
      // A complete protocol frame is enough to retire this one-request socket. Intentionally
      // leave the server half open so the test cannot succeed by depending on its FIN.
      socket.write(`${JSON.stringify({ id: request.id, result: { received: true } })}\n`);
    });
    socket.on("error", () => {});
    socket.once("close", () => { sockets.delete(socket); peerClosed(); });
  });
  try {
    await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(endpoint, resolve); });
    await expect(callTurnBroker(endpoint, { method: "owner_status" })).resolves.toEqual({ received: true });
    expect(clientConnected).toBe(1);
    expect(clientClosed).toBeTrue();
    await peerClosedPromise;
    expect(sockets.size).toBe(0);
  } finally {
    observedConnect.mock.restore();
...
test("broker pipe rejects a mismatched response ID and an incomplete frame", async () => {
  for (const scenario of ["mismatched-id", "truncated-frame"] as const) {
    const root = mkdtempSync(join(shortSocketTempRoot(), "cgw-rejected-frame-"));
    const sockets = new Set<Socket>();
    const endpoint = defaultBrokerEndpoint(root);
    const server = createServer(socket => {
      sockets.add(socket);
      ...
      socket.on("data", chunk => {
        received += chunk;
        if (!received.includes("\n")) return;
        socket.end(scenario === "mismatched-id"
          ? `${JSON.stringify({ id: "different-request", result: { mustNotSucceed: true } })}\n`
          : '{"id":');
      });
      socket.on("error", () => {});
    ...
    try {
      await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(endpoint, resolve); });
      await expect(callTurnBroker(endpoint, { method: "owner_status" })).rejects.toThrow(
        scenario === "mismatched-id" ? "response id mismatch" : "closed the connection",
      );
    } finally {
      for (const socket of sockets) socket.destroy();
      if (server.listening) await new Promise<void>((resolve, reject) => server.close(error => error ? reject(error) : resolve()));
...
test("a finished browser without a control submission fails without waiting for the compaction deadline", async () => {
  const sourceRequest = request(false);
  const source = new ChatGptTurnSession({
  ...
  await expect(requestRetainedCompactionHandoff(
    { run: async () => "Connector was unavailable" } as never, request(true), source, broker,
    { localToolsEnabled: true, solAvailable: true, proAvailable: true }, "trace_missing_handoff", undefined, 60_000,
  )).rejects.toThrow("finished without submitting");
