...
function reasoningPicker(options: { max?: string; delay?: number; missing?: boolean } = {}) {
  ...
  // Current picker: pressing the wrapper does not dispatch to the child slider.
  const sliderControl = { press: async () => { throw new Error("Menu wrapper cannot change the slider"); } };
  const slider = {
...
test("Pro selection focuses the semantic slider instead of its non-forwarding menu wrapper", async () => {
