...
test("daemon streams browser lifecycle through the real helper process", async () => {
  ...
  writeFileSync(helper, `
    ...
    ChatGptBrowserWorker.prototype.run = async function(turn) {
      if (this.config.accountIdentity?.userId !== "user-pinned" || this.config.modelBinding?.webModelSlug !== "actual-pinned-model") {
        throw new Error("Fixed account/model binding was lost crossing the real helper process boundary");
      }
  ...
  const config: ResolvedBrowserConfig = {
    ...
    accountIdentity: { label: "Pinned", userId: "user-pinned", accountId: "workspace-pinned" },
    modelBinding: { accountId: "pinned", publicModelId: "chatgpt-web/pinned/test", webModelSlug: "actual-pinned-model" },
