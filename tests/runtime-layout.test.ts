...
test("Windows uses a stable native named pipe for the outer Codex tool broker", () => {
  const first = defaultBrokerEndpoint("C:\\Users\\alice\\.morecodex", "win32");
  const second = defaultBrokerEndpoint("C:\\Users\\alice\\.morecodex", "win32");
  expect(first).toBe(second);
  expect(isWindowsPipeEndpoint(first)).toBe(true);
  expect(resolveBrokerEndpoint(first)).toBe(first);
  expect(defaultBrokerEndpoint("/home/alice/.morecodex", "linux")).toEndWith(join("runtime", "turn-broker.sock"));
});
...
test("launcher browser ownership is explicit in provider configuration", () => {
  ...
  config.browserHostDescriptorPath = "/Users/example/.morecodex/runtime/launcher-browser.json";
