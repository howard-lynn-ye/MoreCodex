...
import { TUNNEL_VERSION, mcpCommand, parseTunnelStatus, tunnelClientInstallAction, tunnelCommandOutput, tunnelConnectLaunchError, tunnelInventoryMatchesBroker } from "../src/tunnel";
import type { AppConfig } from "../src/config";

test("route readiness requires the live MCP broker, including when native inventory merges aliases", () => {
  const config = { mode: "full", brokerSocketPath: "\\\\.\\pipe\\current-home", runtimeCommand: ["C:\\runtime\\bun.exe", "C:\\runtime\\cli.js"], tunnel: { alias: "current", tunnelId: "tunnel-current" } } as AppConfig;
  const live = { found: true, status: { control_plane_tunnel_id: "tunnel-current", channels: [{ name: "main", details: [{ key: "command", value: mcpCommand(config, "win32") }] }] } };
  const entries = [{ alias: "current", live_runtime: { found: false } }, { alias: "old-alias", live_runtime: live }];
  expect(tunnelInventoryMatchesBroker(JSON.stringify({entries}), config, "win32")).toBe(true);
  live.status.channels[0]!.details[0]!.value = mcpCommand({ ...config, brokerSocketPath: "\\\\.\\pipe\\old-home" }, "win32");
  expect(tunnelInventoryMatchesBroker(JSON.stringify({entries}), config, "win32")).toBe(false);
  expect(tunnelInventoryMatchesBroker(JSON.stringify({entries: []}), config, "win32")).toBe(false);
});
