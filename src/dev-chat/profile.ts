...
const WINDOWS_LAUNCHER_GUID = "f3a25aab-fc95-4f2d-9fc2-345abce01694";
...
export function resolveDevProfilePaths({
  ...
  const home = resolve(expandUserPath(
    environment.CODEX_WEB_GPT_DEV_HOME?.trim() || join(homeDirectory, ".morecodex-dev"),
  ));
  const productionHome = resolve(expandUserPath(
    environment.CODEX_CHATGPT_WEB_HOME?.trim() || join(homeDirectory, ".morecodex"),
  ));
...
export function installedLauncherCandidates({
  ...
  if (platform === "darwin") {
    candidates.push(
      "/Applications/MoreCodex.app/Contents/MacOS/MoreCodex",
      posix.join(homeDirectory, "Applications", "MoreCodex.app", "Contents", "MacOS", "MoreCodex"),
    );
    ...
    if (registeredLocation && win32.isAbsolute(registeredLocation)) {
      candidates.push(win32.join(registeredLocation, "MoreCodex.exe"));
    } else {
      const localAppData = environment.LOCALAPPDATA?.trim();
      if (localAppData) {
        candidates.push(win32.join(localAppData, "Programs", "MoreCodex", "MoreCodex.exe"));
      }
    }
...
export function findInstalledLauncherExecutable(options: Parameters<typeof installedLauncherCandidates>[0] = {}): string {
  ...
  throw new Error(
    "Installed MoreCodex launcher was not found. Install it first or set CODEX_WEB_GPT_LAUNCHER_EXECUTABLE to its absolute executable path."
