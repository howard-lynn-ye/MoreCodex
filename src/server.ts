...
import { resolveWebAccountModel, readWebAccountRegistry } from "./web-accounts";
import { readFileSync as readWebCatalogFile } from "node:fs";
import { forwardHttpAccountRequest, httpAccountModels, HttpAccountError, HTTP_ACCOUNT_PREFIX } from "./http-accounts";
...
// Codex may reconnect after response.failed even when retryable is false. Once
// headers have streamed, reject an exact replay at HTTP level before any browser
// work. Trace IDs include the provider/account and native turn/revision identity.
const terminalReplays = new Map<string, { expires: number; status: number; error: { type: string; code: string | null; message: string } }>();
function terminalReplay(traceId: string | undefined) {
  const now = Date.now();
  for (const [key, value] of terminalReplays) if (value.expires <= now) terminalReplays.delete(key);
  return traceId ? terminalReplays.get(traceId) : undefined;
}
function rememberTerminalReplay(traceId: string | undefined, event: AdapterEvent) {
  if (!traceId || event.type !== "error" || event.retryable !== false) return;
  const status = event.status ?? (event.errorType === "invalid_request_error" ? 400 : undefined);
  if (status === undefined || status < 400 || status >= 500 || status === 429) return;
  terminalReplay(undefined);
  if (!terminalReplays.has(traceId) && terminalReplays.size >= 256) terminalReplays.delete(terminalReplays.keys().next().value!);
  terminalReplays.set(traceId, { expires: Date.now() + 5 * 60_000, status,
    error: { type: event.errorType ?? "invalid_request_error", code: event.code ?? null, message: event.message } });
}
...
export async function modelsRequest(
  ...
  if (config.httpOnly) {
    try { return Response.json({ models: httpAccountModels(config) }); }
    catch { return formatErrorResponse(503, "account_registry_unavailable", "HTTP account catalogue is unavailable"); }
  }
  const independentWebCatalog = (): Response | undefined => {
    const registry = readWebAccountRegistry(config);
    if (!registry?.nativeCatalogFile) return undefined;
    const native = JSON.parse(readWebCatalogFile(registry.nativeCatalogFile, "utf8"));
    const augmented = augmentNativeModelCatalog(native, config, contextOverride?.());
    return Response.json(augmented, { headers: { "cache-control": "no-store" } });
  };
  // An enrolled Web directory is local to this Windows user, not to the active Codex login.
  // Supply it before a native authentication/network failure can hide its entries.
  let independent: Response | undefined;
  try { independent = independentWebCatalog(); }
  catch { return formatErrorResponse(503, "account_registry_unavailable", "Web account catalogue is unavailable; check web-accounts status"); }
  ...
  try {
    ...
    if (independent) return independent;
    const accountModels = httpAccountModels(config);
    if (accountModels.length) return Response.json({ models: accountModels });
    return formatErrorResponse(502, "upstream_error", error instanceof Error ? error.message : String(error));
  }
  if (!upstream.ok) {
    if (independent) { await upstream.body?.cancel(); return independent; }
    const accountModels = httpAccountModels(config);
    if (accountModels.length) { await upstream.body?.cancel(); return Response.json({models:accountModels}); }
    return upstream;
  }
...
export async function responseRequest(
  ...
  const requestedModel = raw && typeof raw === "object" && !Array.isArray(raw)
    ...
    : undefined;
  if (typeof requestedModel === "string" && requestedModel.startsWith(HTTP_ACCOUNT_PREFIX)) {
    try {
      ...
      return await forwardHttpAccountRequest(req, config, raw as Record<string, unknown>, "responses");
    }
    catch (error) { return formatErrorResponse(error instanceof HttpAccountError ? error.status : 502, "account_error", error instanceof HttpAccountError ? error.message : "HTTP account request failed"); }
  }
  if (config.httpOnly && typeof requestedModel === "string" && isChatGptWebModelSlug(requestedModel)) return formatErrorResponse(409, "invalid_request_error", "Browser model routes are disabled in this HTTP-only runtime");
  try {
    const identity = extractCodexTurnIdentityFromBody(raw);
  ...
  try {
    ...
    const selected = resolveWebAccountModel(parsed.modelId, config);
    config = selected.config;
    parsed.modelId = selected.model;
    route = routeChatGptWebRequest(parsed, config);
    if (selected.account) route = { ...route, slug: requestedModel as string };
  ...
  const priorFailure = terminalReplay(traceId);
  if (priorFailure) return Response.json({ error: priorFailure.error }, { status: priorFailure.status });
  ...
  const run = async () => {
    try {
      await adapter.runTurn!(parsed, { headers: req.headers, abortSignal: abort.signal }, event => {
        rememberTerminalReplay(traceId, event);
...
export async function compactRequest(
  ...
  if (raw.model.startsWith(HTTP_ACCOUNT_PREFIX)) {
    try { return await forwardHttpAccountRequest(req, config, raw as Record<string, unknown>, "responses/compact"); }
    catch (error) { return formatErrorResponse(error instanceof HttpAccountError ? error.status : 502, "account_error", error instanceof HttpAccountError ? error.message : "HTTP account compaction failed"); }
  }
  if (config.httpOnly && isChatGptWebModelSlug(raw.model)) return formatErrorResponse(409, "invalid_request_error", "Browser model routes are disabled in this HTTP-only runtime");
  ...
  let route: ChatGptWebModelRoute;
  try {
    const selected = resolveWebAccountModel(raw.model, config);
    route = requireChatGptWebModelRoute(selected.model, selected.config);
  } catch (error) {
    return formatErrorResponse(400, "invalid_request_error", error instanceof Error ? error.message : String(error));
  }
    ...
    async fetch(req) {
      ...
      if (req.method === "GET" && url.pathname === "/healthz") {
        return Response.json({
          ...
          version: VERSION,
          mode: config.mode,
          inference_transport: config.httpOnly ? "http-only" : "mixed",
          pid: process.pid,
          port: config.port,
