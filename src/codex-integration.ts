import { assertOwnedCatalogFile, prepareCodexModelCatalog, serializeCodexModelCatalog,
  verifyActiveCodexModelCatalog } from "./codex-integration-catalog";
export { getCodexManagedCatalogPath } from "./codex-integration-catalog";
...
export function preflightCodexIntegration(
  ...
  if (existing && existing.version !== 2) {
    ...
    if (existing.version === 10 || existing.version === 11) return;
...
export function installCodexIntegration(
  ...
  if (hasManagedJournal && existing && existing.version !== 2) {
    ...
    const priorCatalog = existing.version === 11 ? existing.modelCatalog : undefined;
    const catalog = options.modelCatalog !== undefined || priorCatalog
      ? prepareCodexModelCatalog(patched.text, priorCatalog, options.modelCatalog) : undefined;
    const updated: CodexIntegrationJournal = {
      ...(catalog ? { version: 11 as const, modelCatalog: catalog.record } : { version: 10 as const }),
      active: true,
      configPath,
      installed: {
    ...
    writeIntegrationState(updated, { path: configPath, data: catalog?.text ?? patched.text }, [getCodexModelsCachePath()], catalog ? [catalog.write] : []);
    return updated;
  }
  ...
  const catalog = options.modelCatalog !== undefined ? prepareCodexModelCatalog(patched.text, undefined, options.modelCatalog) : undefined;
  ...
  writeIntegrationState(journal, { path: configPath, data: catalog?.text ?? patched.text }, [getCodexModelsCachePath()], catalog ? [catalog.write] : []);
  if (existing?.version === 2 && existsSync(existing.catalogPath)) rmSync(existing.catalogPath);
  return journal;
}

/** Regenerate from the same native + verified-account catalogue returned by /models. */
export function refreshCodexModelCatalog(value: unknown): { changed: boolean; enabled: boolean; path?: string } {
  const journal = readJournal();
  if (!journal || journal.version !== 11 || !journal.modelCatalog || !journal.active) return { changed: false, enabled: false };
  assertJournalTargetsConfig(journal, getCodexConfigPath());
  verifyInstalledRoute(readFileSync(journal.configPath, "utf8"), journal);
  verifyActiveCodexModelCatalog(readFileSync(journal.configPath, "utf8"), journal.modelCatalog);
  const data = serializeCodexModelCatalog(value);
  const fingerprint = sha256(data);
  if (fingerprint === journal.modelCatalog.sha256) return { changed: false, enabled: true, path: journal.modelCatalog.path };
  const updated = { ...journal, modelCatalog: { ...journal.modelCatalog, sha256: fingerprint } };
  writeIntegrationState(updated, undefined, [], [{ path: journal.modelCatalog.path, data }]);
  return { changed: true, enabled: true, path: journal.modelCatalog.path };
}
...
export function activateCodexIntegration(): SetCodexIntegrationActiveResult {
  ...
  const catalog = existing.version === 11 && existing.modelCatalog
    ? prepareCodexModelCatalog(route.text, existing.modelCatalog) : undefined;
  ...
  writeIntegrationState(connected, { path: existing.configPath, data: catalog?.text ?? route.text }, [getCodexModelsCachePath()], catalog ? [catalog.write] : []);
...
export function uninstallCodexIntegration(): UninstallCodexIntegrationResult {
  ...
  let ownedCatalogPath: string | undefined;
  if (journal.version === 11 && journal.modelCatalog) {
    try {
      assertOwnedCatalogFile(journal.modelCatalog);
      // A later user edit may deliberately retain the same file with a changed comment.
      // Never remove a file that the restored configuration still references.
      if (findTopLevelAssignment(splitLines(restored), "model_catalog_json").value !== journal.modelCatalog.path) {
        ownedCatalogPath = journal.modelCatalog.path;
      }
    } catch { /* Preserve user-modified files. */ }
  }
  const catalogSnapshot = journal.version === 2 ? snapshotFile(journal.catalogPath)
    : ownedCatalogPath ? snapshotFile(ownedCatalogPath) : undefined;
  ...
  if (journal) {
    try {
      ...
      if ((journal.version === 4 || journal.version === 5 || journal.version === 6 || journal.version === 7 || journal.version === 8 || journal.version === 9 || (journal.version === 10 || journal.version === 11)) && !journal.active) {
        ...
        if (journal.version === 11) verifyActiveCodexModelCatalog(text, journal.modelCatalog);
      }
      else {
