...
import { httpAccountCommand } from "./http-account-cli";
import { webAccountCommand } from "./web-account-cli";
import { readWebAccountRegistry } from "./web-accounts";
...
import { installRuntimeKeyBytes, managedRuntimeKeyPath, stopTunnel, tunnelStatus, verifyTunnelBrokerBinding, waitForTunnelReady } from "./tunnel";
...
Usage:
  ...
  codex-chatgpt-web route <status|connect|disconnect|refresh-catalog>
  codex-chatgpt-web subagents <status|compatibility-v1|native>
  codex-chatgpt-web browser check
  codex-chatgpt-web http-accounts <status|check>
  codex-chatgpt-web web-accounts <add|attach-browser|start|login|stop|discover|model|rename|enable|disable|catalog|status>
...
async function loginCommand(args: string[]): Promise<void> {
  ...
  if (!launcherControl) {
    ...
    if (config.browserHost === "launcher") {
      throw new Error("ChatGPT login is owned by the launcher; open MoreCodex and use its Sign in step");
    }
...
async function setupCommand(args: string[]): Promise<void> {
  ...
  options.deferCodexRoute = takeFlag(args, "--defer-codex-route");
...
async function routeCommand(args: string[]): Promise<void> {
  ...
  assertNoArgs(args);
  const configuredCatalog = (config: ReturnType<typeof loadConfig>) => {
    const registry = readWebAccountRegistry(config);
    if (!registry) return undefined;
    if (!registry.nativeCatalogFile) throw Error("Account integration requires the official native catalogue template; run web-accounts catalog first");
    return augmentNativeModelCatalog(JSON.parse(readFileSync(registry.nativeCatalogFile, "utf8")), config);
  };
  if (action === "refresh-catalog") {
    const catalog = configuredCatalog(loadConfig());
    if (!catalog) throw Error("No Web account catalogue is configured");
    stdout.write(`${JSON.stringify(refreshCodexModelCatalog(catalog))}\n`);
    return;
  }
  if (action === "install" || action === "connect") {
    const config = loadConfig();
    const healthUrl = `http://${config.host}:${config.port}/healthz`;
    const response = await fetch(healthUrl, { signal: AbortSignal.timeout(3000) }).catch(() => {
      throw new Error(`Cannot connect Codex: local bridge ${healthUrl} is unavailable. Start the bridge and retry; the current route was kept.`);
    });
    const health = await response.json() as Record<string, unknown>;
    if (!response.ok || health.service !== "codex-chatgpt-web" || health.status !== "ok"
      || health.version !== config.releaseVersion || health.mode !== config.mode || health.accepting_turns !== true) {
      throw new Error("Cannot install Codex route before the configured Responses runtime is ready");
    }
    if (config.mode === "full" && !tunnelStatus(config).ready) throw new Error("Cannot install Codex route before Tunnel readiness");
    if (config.mode === "full" && !verifyTunnelBrokerBinding(config)) {
      throw new Error("Cannot connect Codex: the live Tunnel MCP command is not bound to this bridge's broker");
    }
    const modelCatalog = configuredCatalog(config);
    if (action === "connect" && modelCatalog === undefined) {
      stdout.write(`${JSON.stringify(activateCodexIntegration())}\n`);
      return;
    }
    // Reconnecting after account edits must atomically install the current catalogue,
    // not reactivate the older directory saved when the integration was disconnected.
    installCodexIntegration(config, modelCatalog ? { modelCatalog } : undefined);
    const status = inspectCodexIntegration();
    stdout.write(`${JSON.stringify({ installed: status.installed, active: status.active, errors: status.errors })}\n`);
    return;
  }
...
async function uninstallCommand(args: string[]): Promise<void> {
  ...
  if (config?.browserHost === "launcher" && !launcherControl) {
    throw new Error(
      "Launcher-owned integration must be removed from MoreCodex Settings so the active runtime can be drained safely.",
    );
  }
...
async function main(): Promise<void> {
  ...
  else if (command === "http-accounts") await httpAccountCommand(args);
  else if (command === "web-accounts") await webAccountCommand(args);
