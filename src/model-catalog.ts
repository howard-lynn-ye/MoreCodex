...
import { configForWebAccount, configForWebModel, readWebAccounts, verifiedAccountModels, webModelId } from "./web-accounts";
import { httpAccountModels } from "./http-accounts";
import {
...
export function buildChatGptWebModel(
  ...
  const accountLabel = config.modelAccountLabels
    ? config.modelAccountLabels[config.tunnel?.tunnelId ?? ""] ?? "ACCOUNT UNVERIFIED"
    : undefined;
  const displayName = accountLabel
    ? `CODEX WEB ${route.displayName.replace(/^ChatGPT Web — /, "").toUpperCase()} ${accountLabel.toUpperCase()}`
    : route.displayName;
  const model: JsonObject = {
    ...
    description: accountLabel
      ? `${route.description} ChatGPT web account: ${accountLabel}.`
      : route.description,
    ...
    supported_reasoning_levels: [reasoningLevel(template, route.codexEffort, displayName)],
  ...
  if (route.backendModel === CHATGPT_WEB_CONFIGURED_BACKEND_MODEL) {
    const definition = config.webModelDefinition;
    if (!definition || config.webModelBinding?.publicModelId !== route.slug
      || config.webModelBinding.webModelSlug !== definition.slug) {
      throw new Error("Configured model catalogue entry has no matching account/model binding");
    }
    model.input_modalities = [...definition.capabilities.inputModalities];
    model.supports_tools = definition.capabilities.tools;
    // The native row is a protocol/schema template, not evidence of these Web capabilities.
    ...
    model.multi_agent_version = "disabled";
    model.use_responses_lite = false;
    if (!definition.capabilities.tools) model.node_repl_disabled = true;
...
export function augmentNativeModelCatalog(
  ...
  const accounts = config.httpOnly ? undefined : readWebAccounts(config);
  const webModels = config.httpOnly ? [] : accounts
    ? accounts.filter(account => account.enabled && account.userId && account.accountId).flatMap(account => {
      const accountConfig = configForWebAccount(config, account);
      if (account.models !== undefined) return verifiedAccountModels(account).map(entry => {
        const selectedConfig = configForWebModel(config, account, entry);
        const route = requireChatGptWebModelRoute(entry.definition ? webModelId(account, entry) : entry.adapterRoute!, selectedConfig);
        const model = buildChatGptWebModel(template, route, selectedConfig);
        const displayName = `${entry.title} ${account.label}`;
        return { ...model, slug: webModelId(account, entry), display_name: displayName,
          description: `ChatGPT Web browser transport. Fixed account: ${account.label}. Actual model: ${entry.slug}. No fallback.`,
          supported_reasoning_levels: [reasoningLevel(template, route.codexEffort, displayName)],
        };
      });
      return availableChatGptWebModelRoutes(accountConfig).map(route => {
        const model = buildChatGptWebModel(template, route, accountConfig);
        const displayName = `ChatGPT Web ${route.displayName.replace(/^ChatGPT Web — /, "")} ${account.label.toUpperCase()}`;
        return {
          ...model,
          slug: `${CHATGPT_WEB_MODEL_PREFIX}${account.id}/${route.slug.slice(CHATGPT_WEB_MODEL_PREFIX.length)}`,
          display_name: displayName,
          description: `${route.description} ChatGPT web account: ${account.label}.`,
          supported_reasoning_levels: [reasoningLevel(template, route.codexEffort, displayName)],
        };
      });
    })
  ...
  return {
    ...
    models: [...nativeModels, ...(config.httpOnly ? [] : webModels), ...httpAccountModels(config)],
  };
}
