  ...
  try {
    ...
    const browserFailure = browser.then<never>(
      () => { throw new ChatGptWebAdapterError(
        "ChatGPT finished without submitting the required compaction checkpoint through this account's connector. Check its Tunnel connection before retrying.",
        { status: 400, errorType: "invalid_request_error", code: "compaction_handoff_missing", retryable: false },
      ); },
      error => { throw error; },
    );
