...
export function chatGptReadOnlyContextWarning(
  ...
  const label = capabilities.webModelDefinition?.title ?? (mode.effort === "max" ? "ChatGPT Pro" : `ChatGPT Web ${mode.displayLabel}`);
  ...
  const browserOnlyGuidance = !capabilities.localToolsEnabled && !capabilities.webModelDefinition
    ? "\n>\n> **Action:** Open `MCP` in `MoreCodex` and connect the `Full` harness to give the selected ChatGPT Web model access to local tools."
    : "";
  if (hasLocalEvidence) {
...
export function compileChatGptWebPrompt(
  ...
  if (capabilities.webModelDefinition && (manualControl || multipartEnabled)) {
    throw new Error("Configured Web models do not support manual or legacy multipart transport");
  }
  if (capabilities.webModelDefinition && !capabilities.webModelDefinition.capabilities.inputModalities.includes("image")
    && countChatGptContextImages(parsed.context.messages) > 0) {
    throw new ChatGptWebAdapterError("The selected Web model accepts text only; images were not sent", {
      status: 400, errorType: "invalid_request_error", code: "web_model_images_unsupported", retryable: false,
    });
  }
  if (manualControl) {
  ...
  // Configured models have explicit transport limits. Never inherit the legacy preset's
  // 110k-byte trimming heuristic; the worker validates this complete prompt before sending it.
  if (capabilities.webModelDefinition) return compiled;
