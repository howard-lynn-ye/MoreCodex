...
import {
  ...
  validateExternalBrowserOptions,
} from "../../config";
import { ExternalBrowserConnection, ExternalBrowserError, type ExternalBrowserOptions, type ExternalBrowserPageLease } from "../../external-browser";
import { cloneWebModelDefinition, type WebModelDefinition } from "../../web-model-definition";
...
import {
  CHATGPT_WEB_CONFIGURED_BACKEND_MODEL,
  resolveChatGptWebContextLimits,
  resolveChatGptWebMessageTokenBudget,
  resolveChatGptWebTransportLimits,
} from "../../chatgpt-web-models";
import { LauncherBrowserHelperClient } from "./launcher-helper-client";
import { ExternalRetainedPages } from "./external-retained";
import { selectConfiguredWebModel } from "./configured-model-selection";
...
/** Account-scoped transport reuse; changing a model must not ask the browser for a new connection. */
export class ExternalBrowserWorkerPool {
  private readonly entries = new Map<string, { ready: Promise<ExternalBrowserConnection>; users: number }>();
  constructor(private readonly connect = (options: ExternalBrowserOptions) => ExternalBrowserConnection.connect(options)) {}

  acquire(options: ExternalBrowserOptions): { ready: Promise<ExternalBrowserConnection>; release(): Promise<void> } {
    const key = JSON.stringify([
      options.userDataDir, options.email.trim().toLowerCase(), options.userId, options.accountId,
      options.connectTimeoutMs ?? 30_000, options.connectionTtlMs ?? null,
    ]);
    let entry = this.entries.get(key);
    if (!entry) {
      // Keep a rejected promise too: a request must never trigger repeated browser permission prompts.
      entry = { ready: Promise.resolve().then(() => this.connect({ ...options })), users: 0 };
    ...
    return {
      ...
      release: async () => {
        ...
        const connection = await owned.ready.catch(() => undefined);
...
const externalBrowserPool = new ExternalBrowserWorkerPool();
...
export async function throwIfChatGptSessionFailureAlert(page: Page): Promise<void> {
  if (await chatGptExpiredSessionAlert(page).isVisible().catch(() => false)) {
    throw new ChatGptWebAdapterError(
      "The ChatGPT session has expired. Sign in again in MoreCodex.",
...
export function assertChatGptWebInputWithinLimits(
  ...
  if (modelId !== CHATGPT_WEB_MODEL_ID && modelId !== CHATGPT_WEB_LUNA_MODEL_ID && modelId !== CHATGPT_WEB_CONFIGURED_BACKEND_MODEL) {
...
// Pro answers keep the page busy for 6-15 s at a time (measured 2026-09-16); 5 s tore healthy pages down.
export const CHATGPT_BROWSER_OBSERVATION_PROBE_TIMEOUT_MS = 30_000;
...
/** Last time a guarded ChatGPT conversation request/response moved on the network (set by the model guard). */
export const CHATGPT_WEB_ACTIVITY_GRACE_MS = 90_000;
export const CHATGPT_BROWSER_OBSERVATION_MAX_WAIT_MS = 600_000;
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_resolve, reject) => {
        const expire = () => {
          const now = Date.now();
          const lastActivity = (globalThis as { __cwWebActivityAt?: number }).__cwWebActivityAt ?? 0;
          // A page whose ChatGPT answer is still moving on the network is busy rendering, not dead.
          if (timeoutMs >= CHATGPT_BROWSER_OBSERVATION_PROBE_TIMEOUT_MS
            && now - lastActivity < CHATGPT_WEB_ACTIVITY_GRACE_MS
            && now - startedAt < CHATGPT_BROWSER_OBSERVATION_MAX_WAIT_MS) {
            timer = setTimeout(expire, 15_000);
            return;
          }
          reject(new ChatGptBrowserObservationTimeoutError(now - startedAt));
        };
...
import { verifyChatGptAccount, type ChatGptAccountIdentity } from "../../chatgpt-account";
import { installWebModelGuard } from "../../web-model-guard";

export interface ResolvedBrowserConfig {
  appName: string;
  browserHost: "managed-chrome" | "launcher" | "external-cdp";
  browserHostDescriptorPath?: string;
  externalBrowser?: ExternalBrowserOptions;
  accountIdentity?: ChatGptAccountIdentity;
  modelBinding?: import("../../web-accounts").WebModelBinding;
  webModelDefinition?: WebModelDefinition;
...
export function resolveBrowserConfig(provider: CodexProviderConfig): ResolvedBrowserConfig {
  ...
  const webModelDefinition = configured.webModelDefinition ? cloneWebModelDefinition(configured.webModelDefinition) : undefined;
  if (webModelDefinition && (!configured.accountIdentity || !configured.modelBinding
    || configured.modelBinding.webModelSlug !== webModelDefinition.slug)) {
    throw new Error("Configured Web model requires the same enrolled account and actual model binding");
  }
  if (webModelDefinition && configured.browserInteractionMode === "manual") throw new Error("Configured Web models require automatic interaction");
  ...
  if (browserHost === "external-cdp") {
    validateExternalBrowserOptions(configured.externalBrowser);
    const expected = configured.accountIdentity;
    if (!expected?.userId || !expected.accountId
      || configured.externalBrowser.userId !== expected.userId
      || configured.externalBrowser.accountId !== expected.accountId) {
      throw new Error("External browser requests require the enrolled account and workspace identity; run account discovery first");
    }
    if (browserHostDescriptorPath) throw new Error("External browser requests cannot also use a launcher descriptor");
  }
  ...
  return {
    ...
    browserHost,
    ...(webModelDefinition ? { webModelDefinition } : {}),
    ...(browserHost === "external-cdp" ? { externalBrowser: { ...configured.externalBrowser! } } : {}),
    ...(configured.accountIdentity ? { accountIdentity: { ...configured.accountIdentity } } : {}),
    ...(configured.modelBinding ? { modelBinding: { ...configured.modelBinding } } : {}),
...
export class ChatGptBrowserWorker {
  ...
  private externalConnection?: ReturnType<ExternalBrowserWorkerPool["acquire"]>;
  private readonly externalPages = new Map<Page, ExternalBrowserPageLease>();
  private readonly externalRetainedPages = new ExternalRetainedPages(
    () => this.pageForNewTurn(), page => this.releaseTurnPage(page),
  );
  ...
  run(turn: BrowserTurn): Promise<string> {
    ...
    const run = Promise.resolve().then(() => useHelper ? this.launcherHelper!.run(turn) : this.runExclusive(turn))
      .catch(error => { throw this.externalAdapterError(error); });
  ...
  private enqueueMaintenance<T>(name: string, action: () => Promise<T>): Promise<T> {
    const operation = this.maintenanceTail.then(() => {
      ...
      return action().catch(error => { throw this.externalAdapterError(error); }).finally(async () => {
        if (this.config.browserHost === "external-cdp" && this.page) {
          const page = this.page;
          this.page = undefined;
          await this.releaseTurnPage(page);
        }
      });
    });
  ...
  async close(): Promise<void> {
    ...
    const externalCleanupErrors: unknown[] = [];
    try { await this.externalRetainedPages.close(); }
    catch (error) { externalCleanupErrors.push(error); }
    if (this.externalConnection) {
      const releases = await Promise.allSettled([...this.externalPages.keys()].map(page => this.releaseTurnPage(page)));
      for (const release of releases) if (release.status === "rejected") externalCleanupErrors.push(release.reason);
      const connection = this.externalConnection;
      this.externalConnection = undefined;
    ...
    if (externalCleanupErrors.length) throw new AggregateError(externalCleanupErrors, "External browser cleanup did not confirm closure of every owned page");
  }
  ...
  private async ensurePage(): Promise<Page> {
    ...
    if (this.config.browserHost === "external-cdp") {
  ...
  private async ensureManagedBrowser(): Promise<{ browser: Browser; context: BrowserContext }> {
    if (this.config.browserHost !== "managed-chrome") throw new Error("Only managed-chrome may launch a browser or import login storage");
  ...
  private async pageForNewTurn(): Promise<Page> {
    ...
    if (this.config.browserHost === "external-cdp") {
      this.externalConnection ??= externalBrowserPool.acquire(this.config.externalBrowser!);
      const connection = await this.externalConnection.ready;
      connection.assertReady();
      const lease = await connection.createPage();
      this.externalPages.set(lease.page, lease);
      return lease.page;
    }
  ...
  private externalAdapterError(error: unknown): unknown {
    if (!(error instanceof ExternalBrowserError)) return error;
    return new ChatGptWebAdapterError(
      `ChatGPT Web ${this.config.accountIdentity?.label ?? "account"}: ${error.message} Restart the bridge only after restoring this browser connection.`,
      { status: 409, errorType: "invalid_request_error", code: error.code, retryable: false, cause: error },
    );
  }

  private async releaseTurnPage(page: Page): Promise<void> {
    if (this.config.browserHost === "external-cdp") {
      const lease = this.externalPages.get(page);
      if (!lease) throw new Error("External browser page is not owned by this worker");
      try {
        await lease.release();
        this.externalPages.delete(page);
      } catch (error) {
        // Quarantine this account transport when physical tab closure was not confirmed.
        // Other model workers sharing this account then fail assertReady instead of racing it.
        const connection = await this.externalConnection?.ready.catch(() => undefined);
  ...
  private async verifyPageAccount(page: Page): Promise<void> {
    if (this.config.browserHost === "external-cdp") {
      const lease = this.externalPages.get(page);
      if (!lease) throw new Error("External browser page is not owned by this worker");
    ...
    await verifyChatGptAccount(page, this.config.accountIdentity);
  }
  ...
  private async selectModelAndEffort(
    ...
    if (capabilities.webModelDefinition) {
      await selectConfiguredWebModel(page, capabilities.webModelDefinition);
      await captureDiagnostic?.("configured-model-selected");
      return mode;
    }
    ...
    // Focus the semantic slider itself: the menuitem wrapper no longer forwards
    // arrow keys to the slider's keyboard handler in the current ChatGPT picker.
    const sliderControl = effortSlider;
  ...
  private async prepareTemporaryChatSurface(
    ...
    await this.verifyPageAccount(page);
  ...
  private async smokeTestExclusive(abortSignal?: AbortSignal): Promise<{ effort: string; response: string }> {
    const page = await this.ensurePage();
    await this.prepareTemporaryChatSurface(page);
    const account = this.config.webModelDefinition ? { solAvailable: false, proAvailable: false } : await detectChatGptAccountCapabilities(page);
    ...
    const capabilities: ChatGptWebCapabilities = { ...account, localToolsEnabled: false,
      ...(this.config.webModelDefinition ? { webModelDefinition: this.config.webModelDefinition } : {}) };
    const modelId = this.config.webModelDefinition ? CHATGPT_WEB_CONFIGURED_BACKEND_MODEL
      : account.solAvailable ? CHATGPT_WEB_MODEL_ID : CHATGPT_WEB_LUNA_MODEL_ID;
    const reasoning = this.config.webModelDefinition?.effort.adapter ?? (account.solAvailable ? "high" : "low");
    const mode = resolveChatGptWebModelMode(modelId, reasoning, capabilities);
  ...
  private async runExclusive(turn: BrowserTurn): Promise<string> {
    ...
    if (this.config.browserHost === "external-cdp") {
      if (!turn.conversationKey) {
        if (turn.requireRetainedConversation) throw chatGptRetainedConversationUnavailableError();
        return this.runBrowserTurn(turn);
      }
      const lease = await this.externalRetainedPages.acquire(turn.conversationKey, turn.requireRetainedConversation === true);
      let retain = false;
      try {
        if (lease.reused && !turn.prepareResume) throw new Error("Retained external conversation requires a continuation prompt");
        await turn.onPreparedSelected?.(lease.reused);
        const answer = await this.runBrowserTurn(turn, undefined, lease.page, lease.reused);
        retain = turn.retainConversation === true;
        return answer;
      } finally {
        // The returned browser promise cannot settle before the exact owned page is retained
        // or physically closed. Aborts and failed handoffs always retire it.
        await lease.finish(retain, turn.nativeConnector === true || turn.capabilities.localToolsEnabled);
      }
    }
  ...
  async releaseExternalRetainedConversation(conversationKey: string): Promise<void> {
    if (this.config.browserHost !== "external-cdp") throw new Error("External retained release requires the external browser host");
    await this.externalRetainedPages.retire(conversationKey);
  }
  ...
  private async runBrowserTurn(
    ...
    if (turn.nativeConnector && this.config.webModelDefinition?.capabilities.tools === false) {
      throw new ChatGptWebAdapterError("This configured Web model does not support the connector required for structured compaction", {
        status: 400, errorType: "invalid_request_error", code: "web_model_tools_unsupported", retryable: false,
      });
    }
    ...
    let modelGuard: Awaited<ReturnType<typeof installWebModelGuard>> | undefined;
    try {
      if (turn.abortSignal?.aborted) throw new DOMException("ChatGPT web turn aborted", "AbortError");
      ...
      const multipartFinalPrompt = prepared.multipart && multipartTransactionId
        ...
        : undefined;
      const estimatedInputTokens = estimateCompiledChatGptWebInputTokens(prepared, turn.modelId, browserCapabilities.webModelDefinition);
      const estimatedMessageTokens = estimateCompiledChatGptWebMessageTokens(prepared, browserCapabilities.webModelDefinition?.slug ?? turn.modelId);
      ...
      let page = await this.runStage(turn.traceId, "browser_page", browserStageTimeouts.browserPage, async (abortSignal) => {
        ...
        if (!launcherSurfaceId) {
          ...
          if (abortSignal.aborted) {
            await this.releaseTurnPage(managed).catch(() => {});
      ...
      diagnosticPage = page;
      if (this.config.modelBinding) {
        if (!this.config.accountIdentity) throw Error("A Web model binding requires an enrolled identity");
        modelGuard = await installWebModelGuard(page, this.config.modelBinding, this.config.accountIdentity, turn.traceId);
      }
      ...
      // A retained page may have been signed out or switched since the preceding turn.
      // Verify before sending even a retained continuation, not only on new pages.
      if (reuseConversation) await this.verifyPageAccount(page);
      ...
      // External browser login belongs to the user. A connector mismatch must be surfaced,
      // never turned into an automatic refresh/login loop, even on our own temporary page.
      let catalogRefreshAvailable = this.config.browserHost !== "external-cdp"
        && mode.localTools && !reuseConversation && !prepared.multipart;
      ...
      if (turnConnection) {
        ...
        if (this.config.browserHost === "external-cdp") {
          // A successful reply does not waive physical ownership. Propagate unconfirmed closure
          // so its task cannot be treated as successfully settled while the old page is active.
          await this.releaseTurnPage(managedPage);
        } else await this.releaseTurnPage(managedPage).catch(error => {
          console.error(
