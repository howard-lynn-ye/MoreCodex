...
import type { WebModelDefinition } from "../../web-model-definition";
...
export function estimateCompiledChatGptWebInputTokens(
  ...
  const imageTokens = estimateChatGptWebImageTokens(compiled, definition);
  modelId = definition?.slug ?? modelId;
  ...
  const acknowledgementTokens = compiled.multipart
    ...
    : 0;
  return (definition?.limits.platformReserveTokens ?? CHATGPT_WEB_PLATFORM_RESERVE_TOKENS) + messageTokens + acknowledgementTokens + imageTokens;
}

export function estimateChatGptWebImageTokens(compiled: CompiledChatGptWebPrompt, definition?: WebModelDefinition): number {
  if (definition) {
    if (compiled.images.length && !definition.capabilities.inputModalities.includes("image")) {
    ...
    return compiled.images.length * definition.limits.imageTokenReserveTokens;
  }
