...
export function estimateChatGptWebInputTokens(
  ...
  return estimateCompiledChatGptWebInputTokens(compiled, parsed.modelId, capabilities.webModelDefinition);
}
...
export function resolveBiggerContextMultipartParts(
  ...
  if (capabilities.webModelDefinition) throw new Error("Configured Web models do not support the legacy Bigger Context transport");
...
export function estimateChatGptWebUsage(
  ...
  const outputTokens = conservativeTextTokens(roundEvidenceText(evidence), capabilities.webModelDefinition?.slug ?? parsed.modelId);
  return {
    inputTokens,
    outputTokens,
