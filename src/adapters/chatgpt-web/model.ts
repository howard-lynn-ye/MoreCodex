...
export function resolveChatGptWebModelMode(
  ...
  if (modelId === CHATGPT_WEB_CONFIGURED_BACKEND_MODEL) {
    validateWebModelDefinition(capabilities.webModelDefinition);
    const definition = capabilities.webModelDefinition;
    if (reasoning !== definition.effort.adapter) throw new Error("Configured Web model effort differs from its immutable definition");
    return {
      modelId, effort: definition.effort.adapter, displayLabel: definition.title,
      uiEffortIndex: null, thinkEnabled: false,
      localTools: capabilities.localToolsEnabled && definition.capabilities.tools,
    };
  }
  if (capabilities.webModelDefinition) throw new Error("A configured Web model must use its dedicated internal route");
