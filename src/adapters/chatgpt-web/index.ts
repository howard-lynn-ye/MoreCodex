...
export function createChatGptWebAdapter(
  ...
  const configuredCapabilities: ChatGptWebCapabilities = {
    localToolsEnabled: provider.chatgptWeb?.localToolsEnabled === true && provider.chatgptWeb?.webModelDefinition?.capabilities.tools !== false,
    solAvailable: provider.chatgptWeb?.solAvailable !== false,
    proAvailable: provider.chatgptWeb?.proAvailable === true,
    ...(provider.chatgptWeb?.webModelDefinition ? { webModelDefinition: cloneWebModelDefinition(provider.chatgptWeb.webModelDefinition) } : {}),
  };
  ...
  const retainedLauncherDescriptor = provider.chatgptWeb?.browserHost === "launcher"
    && provider.chatgptWeb.browserHostDescriptorPath
      ...
      : undefined;
  const externalRetention = provider.chatgptWeb?.browserHost === "external-cdp";
  const supportsRetainedConversation = Boolean(retainedLauncherDescriptor || externalRetention);
  ...
  const startRuntime = (
    ...
    // Correlate the native client turn with the browser guard without logging prompts or credentials.
    if (provider.chatgptWeb?.modelBinding) console.info("[web-routing] " + JSON.stringify({
      at: new Date().toISOString(), event: "native_request_bound", traceId,
      ...provider.chatgptWeb.modelBinding,
      threadId: identity.threadId ?? null, turnId: identity.turnId ?? null,
    ...
    const releaseRetainedConversation = conversationKey && supportsRetainedConversation
      ? async () => {
        if (externalRetention) await worker.releaseExternalRetainedConversation(conversationKey);
  ...
  return {
    ...
    async runTurn(parsed, incoming, emit) {
      const runChatGptWebTurn = async (): Promise<void> => {
        ...
        if (configuredCapabilities.webModelDefinition?.capabilities.tools === false && (parsed.context.tools?.length ?? 0) > 0) {
          emit({ type: "error", message: "The selected Web model definition does not support Codex tools; no browser request was submitted.",
        ...
        if (parsed._compactionRequest) {
          ...
          if (structuredCompactionRequired
            && (!supportsRetainedConversation || (!manualRequest && !structuredBroker))) {
            emit({
              type: "error",
          ...
          if (structuredCompactionRequired) {
                  ...
                  const runFreshCompactionFallback = async (reason: string): Promise<string> => {
                    if (externalRetention) {
                      throw new ChatGptWebAdapterError(
                        "The external browser no longer owns this conversation's retained page. No fresh page or replacement model was used for compaction.",
            ...
            try {
              ...
              const missingCheckpoint = handoffError instanceof ChatGptWebAdapterError && handoffError.code === "compaction_handoff_missing";
              const externalCompactionError = externalRetention && handoffError instanceof ChatGptWebAdapterError ? handoffError : undefined;
              emit({
                type: "error",
                message: externalCompactionError?.message ?? (missingCheckpoint ? handoffError.message : "ChatGPT did not complete the context handoff. Retry the task."),
                status: externalCompactionError?.status ?? (missingCheckpoint ? 400 : 409),
                errorType: "invalid_request_error",
                code: externalCompactionError?.code ?? (missingCheckpoint ? "compaction_handoff_missing" : "compaction_handoff_failed"),
                retryable: false,
              });
              return;
            }
