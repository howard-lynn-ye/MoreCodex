...
export async function callTurnBroker<T>(
  ...
  return new Promise<T>((resolveCall, rejectCall) => {
    ...
    const finishError = (error: Error) => {
      if (settled) return;
      settled = true;
      console.error(`[chatgpt-web] broker_call_failed ${JSON.stringify({ method: request.method,
        elapsedMs: Date.now() - startedAt, errorName: error.name, responseFrameReceived: Boolean(response),
        bufferedChars: buffered.length, newlineReceived: buffered.includes("\n") })}`);
      clearTimeout(timer);
      cleanup();
      socket.destroy();
      rejectCall(error);
    ...
    // Bounded calls still wait for physical socket close before resolving, so callers cannot
    // retire the broker while the response is still in flight.
    ...
    socket.on("data", chunk => {
      ...
      if (settleOnResponseFrame || isWindowsPipeEndpoint(socketPath)) {
        // Bun on Windows can stall a same-process pipe's passive close after delivering a
        // delayed response, even blocking its timeout timer. The complete, validated frame is
        // already consumed; actively retire only this one-request client socket. Bounded calls
        // still resolve from its close event, and broker.close still waits for server peers.
        socket.destroy();
      }
    });
  });
...
export class RemoteTurnBroker implements TurnBrokerOwner {
  ...
  async assertCompatible(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        "The running launcher runtime does not expose the DEV turn-owner protocol; update and restart MoreCodex once before using the working-tree DEV chat"
