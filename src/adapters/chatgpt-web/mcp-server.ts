...
const ZERO_RISK_MCP_INSTRUCTIONS = [
  "For each pasted MoreCodex request, begin with codex_turn_start using the request_id in its request block.",
...
export async function runChatGptMcpServer(options: {
  ...
  const claimTurn = async (
    ...
    try {
      ...
      console.error(`[chatgpt-web-mcp] claim_failed ${JSON.stringify({ toolName,
        errorName: error instanceof Error ? error.name : "unknown" })}`);
      try {
        await settleTurnActivity(turnToken, activityId);
      } catch (cleanupError) {
        console.error(`[chatgpt-web-mcp] claim_cleanup_failed ${JSON.stringify({ toolName,
  ...
  const withClaimedTurn = async <T>(
    ...
    if (contract === "native" && turnToken.startsWith("control_")) {
      // A checkpoint capability never authorizes a work-tool inventory or ordinary task tools.
      // Give actionable guidance without treating a token prefix as authentication.
      throw new Error("This is a reserved compaction token, not a work-turn token. Do not call codex_tool_inventory or work tools. Call codex_tool_call directly with wire_name codex.control.compaction_handoff and the handoff_id and summary from the compaction instruction. That call validates the token.");
    }
  ...
  if (contract === "safe") {
    server.registerTool(
      "codex_turn_start",
      {
        ...
        description: "Connect the request_id included in the pasted MoreCodex request so its Codex tools can be used.",
        inputSchema: {
          request_id: turnTokenSchema,
        },
        outputSchema: {
  ...
  server.registerTool(
    "codex_tool_call",
    {
      ...
      description: afterSafeStart(contract, "Invoke an exact wire_name returned by codex_tool_inventory. The outer Codex runtime performs the call, approvals, and UI lifecycle."
        + (contract === "native" ? " Reserved exception: a compaction instruction supplies wire_name codex.control.compaction_handoff, a control_ token, handoff_id and summary. Submit that exact operation directly without calling codex_tool_inventory first; the reserved token cannot authorize ordinary tools." : "")),
      inputSchema: {
        ...turnReferenceInput(contract),
