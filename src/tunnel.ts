...
export function tunnelInventoryMatchesBroker(output: string, config: AppConfig, platform = process.platform): boolean {
  const settings = tunnel(config);
  try {
    const entries = JSON.parse(output).entries;
    if (!Array.isArray(entries) || !entries.some(entry => entry?.alias === settings.alias)) return false;
    const quote = platform === "win32" ? tunnelCommandQuoted : shellQuote;
    const broker = ["--broker-socket", config.brokerSocketPath].map(quote).join(" ");
    const contract = ["--contract", config.browserInteractionMode === "manual" ? "safe" : "native"].map(quote).join(" ");
    return entries.some(entry => {
      const live = entry?.live_runtime;
      const channel = live?.status?.channels?.find((value: { name?: string }) => value?.name === "main");
      const command = channel?.details?.find((value: { key?: string }) => value?.key === "command")?.value;
      return live?.found === true && live.status?.control_plane_tunnel_id === settings.tunnelId
        && typeof command === "string" && command.endsWith(` ${broker}`) && command.includes(` ${contract} `);
    });
  } catch { return false; }
}

export function verifyTunnelBrokerBinding(config: AppConfig): boolean {
  const result = runCommand(tunnel(config).binaryPath, ["runtimes", "cleanup", "--json"], { timeout: 5_000 });
  return result.status === 0 && tunnelInventoryMatchesBroker(tunnelCommandOutput(result), config);
}
