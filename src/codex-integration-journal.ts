import { assertOwnedCatalogFile, verifyActiveCodexModelCatalog } from "./codex-integration-catalog";
...
function isManagedModelCatalog(value: unknown): boolean {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const record = value as Record<string, unknown>;
  return typeof record.path === "string" && isAbsolute(record.path)
    && typeof record.sha256 === "string" && /^[a-f0-9]{64}$/.test(record.sha256)
    && isPreviousAssignment(record.previous);
}
...
function parseJournal(path: string): AnyCodexIntegrationJournal {
  ...
  if ((value.version === 10 || value.version === 11)
    && (value.version !== 11 || isManagedModelCatalog(value.modelCatalog))
...
function journalMatchesConfig(journal: AnyCodexIntegrationJournal): boolean {
  try {
    ...
    if (journal.version === 11 && journal.modelCatalog) {
      if (journal.active) verifyActiveCodexModelCatalog(text, journal.modelCatalog);
      else assertOwnedCatalogFile(journal.modelCatalog);
    }
    return true;
  } catch {
    return false;
