import { validateWebModelDefinition, type WebModelDefinition } from "./web-model-definition";
...
/** Internal adapter identity; the real model stays in the fixed, verified account binding. */
export const CHATGPT_WEB_CONFIGURED_BACKEND_MODEL = "chatgpt-web-configured";
...
export type ChatGptWebAutomaticBackendModel =
  ...
  | typeof CHATGPT_WEB_CONFIGURED_BACKEND_MODEL;
...
export function resolveChatGptWebContextLimits(
  ...
  if (backendModel === CHATGPT_WEB_CONFIGURED_BACKEND_MODEL || capabilities.webModelDefinition) {
    const definition = configuredDefinition(capabilities);
    return contextLimits(definition.limits.contextWindow, definition.limits.autoCompactTokenLimit);
  }
...
export function resolveChatGptWebTransportLimits(
  ...
  if (backendModel === CHATGPT_WEB_CONFIGURED_BACKEND_MODEL || capabilities.webModelDefinition) {
    const { limits } = configuredDefinition(capabilities);
    return { browserMessageTokenLimit: limits.browserMessageTokenLimit,
      browserComposerCharLimit: limits.browserComposerCharLimit };
  }
...
export function resolveChatGptWebMessageTokenBudget(
  ...
  const reserve = capabilities.webModelDefinition
    ? configuredDefinition(capabilities).limits.platformReserveTokens : CHATGPT_WEB_PLATFORM_RESERVE_TOKENS;
  return Math.max(0, Math.min(
    contextWindow - reserve - imageTokens - 1,
...
export interface ChatGptWebAccountCapabilities {
  ...
  webModelBinding?: { accountId: string; publicModelId: string; webModelSlug: string };
}

function configuredDefinition(capabilities: ChatGptWebAccountCapabilities): WebModelDefinition {
  const definition = capabilities.webModelDefinition;
  if (!definition) throw new Error("Configured ChatGPT Web model requires an explicit model definition");
  validateWebModelDefinition(definition);
  if (capabilities.experimentalBiggerContext) throw new Error("Configured ChatGPT Web model uses its explicit context limits; Bigger Context is unavailable");
  return definition;
}

function configuredRoute(modelId: string, capabilities: ChatGptWebAccountCapabilities): ChatGptWebAutomaticModelRoute {
  const definition = configuredDefinition(capabilities);
  const binding = capabilities.webModelBinding;
  if (!binding || modelId !== binding.publicModelId || binding.webModelSlug !== definition.slug
    || !/^chatgpt-web\/[a-z][a-z0-9-]{0,31}\/m-[a-f0-9]{24}$/.test(modelId)
    || modelId.split("/")[1] !== binding.accountId) {
    throw new Error("Configured ChatGPT Web model does not match its verified account/model binding; no fallback was used");
  }
  if (capabilities.browserInteractionMode === "manual") throw new Error("Configured ChatGPT Web model requires automatic browser transport");
  return { slug: modelId, displayName: definition.title,
    description: `Configured ChatGPT Web model: ${definition.title}. Fixed account and model; no fallback.`,
    interactionMode: "automatic", backendModel: CHATGPT_WEB_CONFIGURED_BACKEND_MODEL,
    codexEffort: definition.effort.codex, adapterEffort: definition.effort.adapter, requiresPro: false };
}
...
export function availableChatGptWebModelRoutes(
  ...
  if (capabilities.webModelDefinition) return [configuredRoute(capabilities.webModelBinding?.publicModelId ?? "", capabilities)];
...
export function requireChatGptWebModelRoute(
  ...
  if (capabilities.webModelDefinition) return configuredRoute(modelId, capabilities);
