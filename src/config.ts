...
import type { ExternalBrowserOptions } from "./external-browser";
import { cloneWebModelDefinition, validateWebModelDefinition, type WebModelDefinition } from "./web-model-definition";
import { VERSION } from "./version";

export type RuntimeMode = "browser-only" | "full";
export type BrowserHostMode = "managed-chrome" | "launcher" | "external-cdp";
export type BrowserInteractionMode = "automatic" | "manual";
export type SubagentProtocol = "compatibility-v1" | "native";

/** Validate only configuration metadata; no browser, endpoint, or login state is accessed. */
export function validateExternalBrowserOptions(value: unknown): asserts value is ExternalBrowserOptions {
  const options = value as Partial<ExternalBrowserOptions> | undefined;
  if (!options || typeof options !== "object" || Array.isArray(options)
    || typeof options.userDataDir !== "string" || !isAbsolute(options.userDataDir)
    || typeof options.email !== "string" || !/^[^\s@]+@[^\s@]+$/.test(options.email.trim())
    || [options.userId, options.accountId].some(id => id !== undefined && (typeof id !== "string" || !id || id.length > 256))
    || [options.connectTimeoutMs, options.connectionTtlMs].some(ms => ms !== undefined
      && (!Number.isSafeInteger(ms) || ms < 1_000 || ms > 7 * 24 * 60 * 60_000))) {
    throw new Error("External browser requires an absolute userDataDir, enrolled email, and valid bounded connection settings");
  }
}
...
export interface AppConfig {
  ...
  /** User-authorized connection to an existing browser; never imports login storage. */
  ...
  /** Display labels for verified account/Tunnel pairs; this does not add model routes. */
  modelAccountLabels?: Record<string, string>;
  /** Local registry of isolated, explicitly verified ChatGPT browser accounts. */
  webAccountsFile?: string;
  /** Model accounts with fixed credentials and HTTPS-only inference. */
  httpAccountsFile?: string;
  httpOnly?: boolean;
  /** Per-request identity from the verified account registry, independent of native Codex login. */
  webAccountIdentity?: ChatGptAccountIdentity;
  webModelBinding?: import("./web-accounts").WebModelBinding;
...
export function getConfigDir(): string {
  ...
  return resolve(expandUserPath(configured || join(homedir(), ".morecodex")));
}
...
function parseConfig(value: unknown, path: string): AppConfig {
  ...
  if (parsed.browserHost !== "managed-chrome" && parsed.browserHost !== "launcher" && parsed.browserHost !== "external-cdp") {
  ...
  if (parsed.webAccountsFile !== undefined
    && (typeof parsed.webAccountsFile !== "string" || !isAbsolute(parsed.webAccountsFile))) {
    throw new Error(`webAccountsFile must be absolute in ${path}`);
  }
  if (parsed.browserHost === "external-cdp") {
    validateExternalBrowserOptions(parsed.externalBrowser);
  }
  if (parsed.webModelDefinition !== undefined) validateWebModelDefinition(parsed.webModelDefinition);
  if (parsed.modelAccountLabels !== undefined) {
    // Existing Web labels remain independent of the HTTP account registry.
    if (!parsed.modelAccountLabels || typeof parsed.modelAccountLabels !== "object"
      || Array.isArray(parsed.modelAccountLabels)
      || Object.entries(parsed.modelAccountLabels).some(([tunnelId, label]) =>
        !/^tunnel_[a-f0-9]{32}$/.test(tunnelId)
        || typeof label !== "string" || !/^[A-Za-z0-9][A-Za-z0-9 _.-]{0,39}$/.test(label))) {
      throw new Error(`Invalid modelAccountLabels in ${path}`);
    }
  }
  if (parsed.mode === "full") {
  ...
  if (parsed.httpAccountsFile !== undefined && (typeof parsed.httpAccountsFile !== "string" || !isAbsolute(parsed.httpAccountsFile))) throw new Error(`httpAccountsFile must be absolute in ${path}`);
  if (parsed.httpOnly !== undefined && typeof parsed.httpOnly !== "boolean") throw new Error(`Invalid httpOnly in ${path}`);
...
export function providerConfig(config: AppConfig): CodexProviderConfig {
  ...
  return {
    ...
    chatgptWeb: {
      ...
      externalBrowser: config.externalBrowser,
      accountIdentity: config.webAccountIdentity,
      modelBinding: config.webModelBinding,
      webModelDefinition: config.webModelDefinition ? cloneWebModelDefinition(config.webModelDefinition) : undefined,
