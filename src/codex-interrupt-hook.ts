...
function powershellArgument(value: string): string {
  ...
  // Native Windows Codex runs hooks through PowerShell. Literal strings preserve
  // spaces, apostrophes, dollar signs and backticks without evaluating path text.
  return `'${value.replaceAll("'", "''")}'`;
}
...
export function codexInterruptHookCommand(
  ...
  const command = args.map(platform === "win32" ? powershellArgument : posixShellArgument).join(" ");
