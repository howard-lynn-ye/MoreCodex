...
const MAX_CACHED_CHUNKS_PER_ESTIMATE = 64;
...
export function estimateTokens(text: string, modelId?: string): number {
  ...
  // Repeated records often contain identical chunks. Cache exact counts only for this call:
  // bounded memory, no cross-request retention of prompt text, and unchanged chunk boundaries.
  ...
  for (let start = 0; start < text.length;) {
    ...
    if (chunkCount === undefined) {
      ...
      if (chunkCounts.size === MAX_CACHED_CHUNKS_PER_ESTIMATE) {
        chunkCounts.delete(chunkCounts.keys().next().value!);
      }
      chunkCounts.set(chunk, chunkCount);
    }
    count += chunkCount;
    start = end;
  }
