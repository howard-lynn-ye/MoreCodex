import { restoreCodexModelCatalog } from "./codex-integration-catalog";
...
export function replacementBaseline(
  ...
  if (journal.version === 9 || (journal.version === 10 || journal.version === 11)) {
    const withoutHook = (journal.version === 10 || journal.version === 11)
      ...
      : currentText;
    const withoutCatalog = journal.version === 11 && journal.modelCatalog
      ? restoreCodexModelCatalog(withoutHook, journal.modelCatalog) : withoutHook;
    const baseline = restoreOwnedManagedFeatures(withoutCatalog, journal);
...
export function restoreManagedRoute(text: string, journal: ManagedRouteJournal): string {
  ...
  const withoutCatalog = journal.version === 11 && journal.modelCatalog
    ? restoreCodexModelCatalog(withoutHook, journal.modelCatalog) : withoutHook;
  const document = parseDocument(withoutCatalog);
  removeManagedComment(document);
  const currentBaseUrl = findTopLevelAssignment(document.lines, "openai_base_url");
